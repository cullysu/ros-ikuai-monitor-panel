...
function Workspace({ route, snapshot, onNavigate, navigationContext }: { route: Exclude<PanelRouteId, "overview" | "lineStatus" | "more">; snapshot: OverviewRawSnapshot; onNavigate: PanelNavigate; navigationContext?: PanelNavigationContext }) {
  ...
  return <main className="ref-mobile ref-workspace" data-mobile-reference-workspace={route}><header className="ref-listbar"><button type="button" onClick={() => onNavigate(PANEL_ROUTES[route].primaryDestination)} aria-label="返回"><ArrowLeft size={20} aria-hidden="true" /></button><h1 data-panel-route-title tabIndex={-1}>{PANEL_ROUTES[route].title}</h1><span className="ref-listbar__badge">{rows.length} 项</span></header><div className="ref-scroll"><div className="ref-content"><WorkspaceTools definition={definition} query={query} filterId={filterId} sortId={sortId} resultCount={filteredRows.length} totalCount={rows.length} onQuery={(value) => { setQuery(value); setPage(1); }} onFilter={(value) => { setFilterId(value); setPage(1); }} onSort={(value) => { setSortId(value); setPage(1); }} /><section className="ref-card ref-object-list"><h2>{definition.objectLabel}</h2>{visibleRows.length ? visibleRows.map((row) => <button key={row.id} type="button" data-panel-object-row={row.id} data-mobile-row-id={row.id} onClick={() => onNavigate(route, { objectId: row.id })}><span><b>{row.primary}</b><small>{row.secondary}</small></span><strong>{row.trailing}</strong><ChevronRight size={16} aria-hidden="true" /></button>) : <p className="ref-empty">当前筛选没有可核实的对象</p>}</section><div className="ref-scroll-sentinel" aria-hidden="true" ref={sentinelRef} /><RouteEvidenceBoundary route={route} model={model} onNavigate={onNavigate} surface="mobile" /><EvidenceWorkspace route={route} model={model} /></div></div></main>;
}
