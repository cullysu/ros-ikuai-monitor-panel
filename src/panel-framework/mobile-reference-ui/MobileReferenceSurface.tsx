...
function EvidenceWorkspace({ route, model }: { route: PanelRouteId; model: ReturnType<typeof buildSectionModel> }) {
  ...
  return <section className="ref-card ref-evidence-workspace" data-mobile-domain-evidence-workspace={route} data-mobile-evidence-workspace-mode={mode} data-mobile-evidence-workspace-actions="recovery-boundary"><h2>数据来源</h2><div className="ref-fact-rows"><div className="ref-fact-row" data-mobile-evidence-workspace-fact="missing"><small>缺失范围</small><b>{missing}</b></div><div className="ref-fact-row" data-mobile-evidence-workspace-fact="impact"><small>影响边界</small><b>{mode === "current" ? "仅当前集合" : mode === "historical" ? "不代表当前" : "业务数字隐藏"}</b></div><div className="ref-fact-row" data-mobile-evidence-workspace-fact="last-success"><small>最近成功业务快照</small><b>{model.updatedAt}</b></div><div className="ref-fact-row" data-mobile-evidence-workspace-fact="collection"><small>当前集合</small><b>{collection}</b></div></div><p className="ref-fact-note" data-mobile-evidence-workspace-investigation>数据问题按下方恢复边界引导处理</p></section>;
}
