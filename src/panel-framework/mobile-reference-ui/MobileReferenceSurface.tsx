import { AlertOctagon, ArrowDown, ArrowLeft, ArrowUp, ArrowDownUp, Check, ChevronDown, ChevronLeft, ChevronRight, Ellipsis, FileText, Grid2X2, Network, RefreshCw, Search, SearchX, Server, SlidersHorizontal, Users, X } from "lucide-react";
...
function Header({ title, alert, onRefresh, onMore }: { title: string; alert?: boolean; onRefresh?: () => void; onMore: () => void }) {
  ...
  return <header className="ref-topbar"><div className="ref-topbar__title">{alert ? <AlertOctagon className="ref-topbar__alert" aria-hidden="true" /> : null}<h1 data-panel-route-title tabIndex={-1}>{title}</h1>{!alert ? <ChevronDown size={15} aria-hidden="true" /> : null}</div><div className="ref-topbar__actions"><button type="button" aria-label={refreshing ? "正在刷新当前数据" : "刷新当前数据"} aria-busy={refreshing} disabled={!onRefresh || refreshing} onClick={refresh}><RefreshCw className={refreshing ? "ref-spin" : undefined} size={19} aria-hidden="true" /></button><button type="button" aria-label="打开更多工具" onClick={onMore}><Ellipsis size={20} aria-hidden="true" /></button></div></header>;
}
...
function ExceptionEvidence({ scene, evidence, snapshot, state, interfaces, onNavigate, onRefresh, onShowConnection }: { scene: Scene; evidence: OverviewEvidenceModel; snapshot: OverviewRawSnapshot; state: OverviewDerivedState; interfaces: InterfaceView[]; onNavigate: PanelNavigate; onRefresh?: () => void; onShowConnection?: () => void }) {
  if (scene === "outage") {
  ...
  if (scene === "unavailable") return <><Facts title="当前边界" rows={[["当前快照", "未取得", "warn"], ["业务速率", "不显示历史值", "muted"], ["下一步核查", "重新请求当前快照", "warn"]]} />{onRefresh ? <button className="ref-inline-action" type="button" onClick={onRefresh}>重新获取快照<RefreshCw size={16} aria-hidden="true" /></button> : null}<section className="ref-card ref-network-actions" data-mobile-reference-recovery-actions aria-label="可用的恢复任务">{onShowConnection ? <button type="button" onClick={onShowConnection}><span><b>检查 RouterOS 连接</b><small>地址、协议与身份验证</small></span><ChevronRight size={16} aria-hidden="true" /></button> : null}<button type="button" onClick={() => onNavigate("serviceLogs")}><span><b>服务日志</b><small>查看采集与服务事件</small></span><ChevronRight size={16} aria-hidden="true" /></button><button type="button" onClick={() => onNavigate("readonlyDiagnostics")}><span><b>只读诊断</b><small>查看当前可读取的诊断项</small></span><ChevronRight size={16} aria-hidden="true" /></button></section></>;
  if (scene === "route") {
...
function Home({ evidence, snapshot, state, onNavigate, onRefresh, onShowConnection }: MobileReferenceProps) {
  ...
  const otherWans = scene === "normal" && wan ? uniqueWan(snapshot).filter((item) => normalized(item.name || item.interface || item.access || item.parent) !== normalized(wan.name)) : [];
 ...
 return <main className="ref-mobile" data-mobile-reference-home data-mobile-reference-scene={scene} data-evidence-mode={evidence.evidenceMode}><Header title={title} alert={alert} onRefresh={onRefresh} onMore={() => onNavigate("more")} /><div className={pull > 0 ? "ref-pull is-active" : "ref-pull"} style={pull > 0 ? { height: Math.min(34, pull * 0.5) } : undefined}>{pull >= 64 ? "释放立即刷新" : "下拉刷新"}</div><div className="ref-scroll" onTouchStart={(event) => { pullStart.current = event.currentTarget.scrollTop <= 0 ? event.touches[0].clientY : null; }} onTouchMove={(event) => { if (pullStart.current === null) return; const delta = event.touches[0].clientY - pullStart.current; if (delta > 0) setPull(Math.min(72, delta)); }} onTouchEnd={() => { if (pull >= 64 && onRefresh) onRefresh(); setPull(0); pullStart.current = null; }}><div className={`ref-content ref-scene-${scene} ${scene === "normal" ? "ref-home-content" : ""}`}><Status title={status[0]} note={status[1]} meta={status[2]} tone={status[3]} />{scene === "normal" && wan ? <><WanCard wan={wan} traffic={traffic} onOpen={() => onNavigate("lineStatus", { objectId: wan.id, returnRoute: "overview", evidenceAt: evidence.evidenceAt })} />{otherWans.length ? <section className="ref-card ref-wanlist" aria-label="其它线路">{otherWans.map((item) => <button key={normalized(item.name || item.interface || item.access || item.parent)} type="button" onClick={() => onNavigate("lineStatus", { objectId: item.name || item.interface, returnRoute: "overview", evidenceAt: evidence.evidenceAt })}><i className="ref-dot" data-tone={item.running === true && item.disabled !== true ? "ok" : "danger"} /><span><b>{item.name || item.interface}</b><small>WAN 线路</small></span><strong data-tone={item.running === true && item.disabled !== true ? "ok" : "danger"}>{item.running === true && item.disabled !== true ? "在线" : "离线"}</strong><ChevronRight size={16} aria-hidden="true" /></button>)}<button className="ref-card-link" type="button" onClick={() => onNavigate("lineStatus")}>全部网络对象<ChevronRight size={15} aria-hidden="true" /></button></section> : null}<div className="ref-home-side"><div className="ref-pair"><Facts title="路由与链路" rows={[["默认路由", wan.verifiedDefault ? wan.name : "待确认", wan.verifiedDefault ? "ok" : "warn"], ["备用链路", `${Math.max(0, uniqueWan(snapshot).length - 1)} 条`], ["策略路由", `${Math.max(0, (snapshot.routes?.defaultRoutes || []).length - 1)} 条`]]} /><Facts title="数据采集" rows={[["来源", evidence.evidenceLabel], ["最后上报", formatRfc3339LocalTime(evidence.evidenceAt) || "未记录"], ["采集通道", state.facts.collection.rest.status === "current" && state.facts.collection.ssh.status === "current" ? "REST + SSH" : "需核对", state.facts.collection.rest.status === "current" && state.facts.collection.ssh.status === "current" ? "ok" : "warn"]]} /></div>{evidence.secondaryDecisions.length ? <Facts title="当前状态" rows={evidence.secondaryDecisions.slice(0, 2).map((item) => [item.object, item.state, item.tone === "danger" ? "danger" : "muted"] as [string, string, Tone])} /> : null}</div></> : null}{scene === "resource" ? <><ResourceList metrics={evidence.resource?.metrics || []} snapshot={snapshot} onOpen={(id) => onNavigate("trafficLoad", { objectId: id, returnRoute: "overview", evidenceAt: evidence.evidenceAt })} /><Facts title="受影响对象" rows={resourceImpactRows(evidence)} /></> : null}{scene === "interfaces" ? <InterfaceList title="接口状态" rows={interfaces} onOpen={(id) => onNavigate("interfaces", { objectId: id, returnRoute: "overview", evidenceAt: evidence.evidenceAt })} onViewAll={() => onNavigate("interfaces")} /> : null}{scene !== "normal" ? <><ExceptionEvidence scene={scene} evidence={evidence} snapshot={snapshot} state={state} interfaces={interfaces} onNavigate={onNavigate} onRefresh={onRefresh} onShowConnection={onShowConnection} /><Facts title="证据与来源" rows={[["数据来源", evidence.evidenceLabel], ["最后上报", formatRfc3339LocalTime(evidence.evidenceAt) || "未记录"], ["证据模式", evidence.evidenceMode === "current" ? "当前" : evidence.evidenceMode === "historical" ? "历史" : "不可用", evidence.evidenceMode === "current" ? "ok" : "warn"]]} /></> : null}{scene !== "outage" ? <TabletTaskBoard evidence={evidence} snapshot={snapshot} state={state} onNavigate={onNavigate} /> : null}</div></div></main>;
}
...
function ResourceEvidenceChart({ evidence }: { evidence: Extract<ReturnType<typeof rowsFromModel>[number]["evidence"], { kind: "resource" }> }) {
  ...
  const threshold = evidence.threshold ?? 100;
  const max = Math.max(100, threshold, ...samples.map((sample) => sample.value));
  ...
  return <section className="ref-card ref-resource-detail-chart"><h2>采样趋势 <span>{samples.length} 点</span></h2><figure aria-label={`${evidence.series || "资源"} 最近 ${samples.length} 个有效采样，当前 ${evidence.latest ?? "未取得"}%，阈值 ${evidence.threshold ?? "未取得"}%`}><div className="ref-resource-detail-unit">%</div><svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="xMidYMid meet" role="img"><title>{evidence.series || "资源"}采样趋势</title><desc>横轴按带时区采样时间排列，实线为资源使用率，虚线为配置阈值。</desc><g className="ref-chart__grid"><line x1={left} y1={top} x2={width - right} y2={top} /><line x1={left} y1={y(max / 2)} x2={width - right} y2={y(max / 2)} /><line x1={left} y1={height - bottom} x2={width - right} y2={height - bottom} /></g><line className="ref-resource-threshold" x1={left} y1={y(threshold)} x2={width - right} y2={y(threshold)} /><path className="ref-resource-series" d={path} /></svg><div className="ref-resource-detail-scale"><span>{Math.round(max)}</span><span>{Math.round(max / 2)}</span><span>0</span></div><div className="ref-chart__footer"><span>{time(start)}</span><span>阈值 {evidence.threshold ?? "—"}%</span><span>{time(end)}</span></div></figure></section>;
}
...
function WorkspaceTools({
  ...
  return <section className="ref-card ref-workspace-tools" aria-label="对象筛选与排序">
    {definition.searchable ? <label className="ref-workspace-search"><Search size={16} aria-hidden="true" /><span className="sr-only">搜索{definition.objectLabel}</span><input type="search" value={query} placeholder={definition.searchPlaceholder} onChange={(event) => onQuery(event.target.value)} />{query ? <button type="button" className="ref-search-clear" aria-label="清空搜索" onClick={() => onQuery("")}><X size={14} aria-hidden="true" /></button> : null}</label> : null}
...
function EvidenceWorkspace({ route, model }: { route: PanelRouteId; model: ReturnType<typeof buildSectionModel> }) {
  const missing = model.metrics.filter((metric) => /未取得|不可用/.test(metric.value)).map((metric) => metric.label).join("、") || model.tables.filter((table) => table.rows.length === 0).map((table) => table.title).join("、") || "未发现";
  const collection = model.tables[0]?.rows.length ? `${model.tables[0].rows.length} 项` : "空集合";
  const mode = model.evidenceMode;
  return <section className="ref-card ref-evidence-workspace" data-mobile-domain-evidence-workspace={route} data-mobile-evidence-workspace-mode={mode} data-mobile-evidence-workspace-actions="recovery-boundary"><h2>数据来源</h2><div className="ref-fact-strip"><div className="ref-fact-cell" data-mobile-evidence-workspace-fact="missing"><small>缺失范围</small><b>{missing}</b></div><div className="ref-fact-cell" data-mobile-evidence-workspace-fact="impact"><small>影响边界</small><b>{mode === "current" ? "仅当前集合" : mode === "historical" ? "不代表当前" : "业务数字隐藏"}</b></div><div className="ref-fact-cell" data-mobile-evidence-workspace-fact="last-success"><small>最近成功业务快照</small><b>{model.updatedAt}</b></div><div className="ref-fact-cell" data-mobile-evidence-workspace-fact="collection"><small>当前集合</small><b>{collection}</b></div></div><p className="ref-fact-note" data-mobile-evidence-workspace-investigation>数据问题按上方边界的引导处理</p></section>;
}

function Workspace({ route, snapshot, onNavigate, navigationContext }: { route: Exclude<PanelRouteId, "overview" | "lineStatus" | "more">; snapshot: OverviewRawSnapshot; onNavigate: PanelNavigate; navigationContext?: PanelNavigationContext }) {
  ...
  useEffect(() => { const node = sentinelRef.current; if (!node) return; const observer = new IntersectionObserver((entries) => { if (entries.some((entry) => entry.isIntersecting)) setPage((current) => Math.min(current + 1, pageCount)); }, { rootMargin: "180px" }); observer.observe(node); return () => observer.disconnect(); }, [pageCount]);
  ...
  if (requestedObject) {
    ...
    return <main className="ref-mobile ref-workspace" data-mobile-reference-workspace={route} data-mobile-reference-object-detail="unavailable"><header className="ref-listbar"><button type="button" onClick={() => onNavigate(route, { objectId: null, replace: true })}><ArrowLeft size={19} aria-hidden="true" />返回</button><h1 data-panel-route-title tabIndex={-1}>{PANEL_ROUTES[route].title}</h1><span /></header><div className="ref-scroll"><div className="ref-content"><Status title="对象证据不可用" note="当前快照没有与该对象标识匹配的记录" meta="不可判" tone="warn" /><button className="ref-operation" type="button" onClick={() => onNavigate(route, { objectId: null, replace: true })}>返回列表</button></div></div></main>;
  }
  return <main className="ref-mobile ref-workspace" data-mobile-reference-workspace={route}><header className="ref-listbar"><button type="button" onClick={() => onNavigate(PANEL_ROUTES[route].primaryDestination)} aria-label="返回"><ArrowLeft size={20} aria-hidden="true" /></button><h1 data-panel-route-title tabIndex={-1}>{PANEL_ROUTES[route].title}</h1><span className="ref-listbar__badge">{rows.length} 项</span></header><div className="ref-scroll"><div className="ref-content"><EvidenceWorkspace route={route} model={model} />{model.metrics.length ? <section className="ref-card ref-metric-grid" data-mobile-metric-grid={route}>{model.metrics.map((metric) => <div key={metric.label} className="ref-metric"><small>{metric.label}</small><b data-tone={metric.tone}>{metric.value}</b></div>)}</section> : null}<WorkspaceTools definition={definition} query={query} filterId={filterId} sortId={sortId} resultCount={filteredRows.length} totalCount={rows.length} onQuery={(value) => { setQuery(value); setPage(1); }} onFilter={(value) => { setFilterId(value); setPage(1); }} onSort={(value) => { setSortId(value); setPage(1); }} /><section className="ref-card ref-object-list"><h2>{definition.objectLabel}</h2>{visibleRows.length ? visibleRows.map((row) => <button key={row.id} type="button" data-panel-object-row={row.id} data-mobile-row-id={row.id} onClick={() => onNavigate(route, { objectId: row.id })}><span><b>{row.primary}</b><small>{row.secondary}</small></span><strong>{row.trailing}</strong><ChevronRight size={16} aria-hidden="true" /></button>) : <p className="ref-empty">当前筛选没有可核实的对象</p>}</section>{model.tables.slice(1).map((tbl, tblIdx) => <section key={tbl.title} className="ref-card ref-secondary-table" data-mobile-secondary-table={tblIdx}><h2>{tbl.title}</h2>{tbl.rows.length ? tbl.rows.slice(0, 30).map((row, rowIdx) => <div key={rowIdx} className="ref-secondary-rows">{tbl.columns.map((col) => { const val = String(row[col.key] ?? ""); return val ? <div key={col.key} className="ref-fact-row"><small>{col.label}</small><b>{val}</b></div> : null; })}</div>) : <p className="ref-empty">{tbl.empty}</p>}</section>)}<RouteEvidenceBoundary route={route} model={model} onNavigate={onNavigate} surface="mobile" /><div className="ref-scroll-sentinel" aria-hidden="true" ref={sentinelRef} /></div></div></main>;
}
...
function Directory({ onNavigate, onShowConnection }: { onNavigate: PanelNavigate; onShowConnection?: () => void }) {
  ...
  return <main className="ref-mobile" data-mobile-reference-directory><header className="ref-listbar"><button type="button" onClick={() => onNavigate("overview")}><ArrowLeft size={19} aria-hidden="true" />返回</button><h1 data-panel-route-title tabIndex={-1}>更多工具</h1><span /></header><div className="ref-scroll"><section className="ref-card ref-object-list">{onShowConnection ? <button type="button" onClick={onShowConnection}><span><b>RouterOS 连接</b><small>设备地址、REST、SSH 与身份验证</small></span><ChevronRight size={16} aria-hidden="true" /></button> : null}{routes.map((route) => <button key={route} type="button" onClick={() => onNavigate(route)}><span><b>{PANEL_ROUTES[route].title}</b><small>{PANEL_ROUTES[route].description}</small></span><ChevronRight size={16} aria-hidden="true" /></button>)}</section></div></main>;
}
...
export function MobileReferenceSurface(props: MobileReferenceProps) {
  ...
  return <Workspace route={props.route} snapshot={props.snapshot} onNavigate={props.onNavigate} navigationContext={props.navigationContext} />;
}
...
const NAV = [
  ...
  { label: "设备", route: "terminals" as PanelRouteId, Icon: Server, active: ["terminals", "dhcp", "arp", "trafficLoad", "loadAudit"] as PanelRouteId[] },
