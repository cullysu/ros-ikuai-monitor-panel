...
function EvidenceWorkspace({ route, model }: { route: PanelRouteId; model: ReturnType<typeof buildSectionModel> }) {
  ...
  return <section className="ref-card ref-evidence-workspace" data-mobile-domain-evidence-workspace={route} data-mobile-evidence-workspace-mode={mode} data-mobile-evidence-workspace-actions="recovery-boundary"><h2>数据来源</h2><div className="ref-fact-rows"><div className="ref-fact-row" data-mobile-evidence-workspace-fact="missing"><small>缺失范围</small><b>{missing}</b></div><div className="ref-fact-row" data-mobile-evidence-workspace-fact="impact"><small>影响边界</small><b>{mode === "current" ? "仅当前集合" : mode === "historical" ? "不代表当前" : "业务数字隐藏"}</b></div><div className="ref-fact-row" data-mobile-evidence-workspace-fact="last-success"><small>最近成功业务快照</small><b>{model.updatedAt}</b></div><div className="ref-fact-row" data-mobile-evidence-workspace-fact="collection"><small>当前集合</small><b>{collection}</b></div></div><p className="ref-fact-note" data-mobile-evidence-workspace-investigation>数据问题按恢复边界引导处理</p></section>;
}
...
function Workspace({ route, snapshot, onNavigate, navigationContext }: { route: Exclude<PanelRouteId, "overview" | "lineStatus" | "more">; snapshot: OverviewRawSnapshot; onNavigate: PanelNavigate; navigationContext?: PanelNavigationContext }) {
  ...
  return <main className="ref-mobile ref-workspace" data-mobile-reference-workspace={route}><header className="ref-listbar"><button type="button" onClick={() => onNavigate(PANEL_ROUTES[route].primaryDestination)} aria-label="返回"><ArrowLeft size={20} aria-hidden="true" /></button><h1 data-panel-route-title tabIndex={-1}>{PANEL_ROUTES[route].title}</h1><span className="ref-listbar__badge">{rows.length} 项</span></header><div className="ref-scroll"><div className="ref-content">{visibleRows.length > 0 ? <EvidenceWorkspace route={route} model={model} /> : null}{model.metrics.length ? <section className="ref-card ref-metric-grid" data-mobile-metric-grid={route}>{model.metrics.map((metric) => <div key={metric.label} className="ref-metric"><small>{metric.label}</small><b data-tone={metric.tone}>{metric.value}</b></div>)}</section> : null}<WorkspaceTools definition={definition} query={query} filterId={filterId} sortId={sortId} resultCount={filteredRows.length} totalCount={rows.length} onQuery={(value) => { setQuery(value); setPage(1); }} onFilter={(value) => { setFilterId(value); setPage(1); }} onSort={(value) => { setSortId(value); setPage(1); }} /><section className="ref-card ref-object-list"><h2>{definition.objectLabel}</h2>{visibleRows.length ? visibleRows.map((row) => <button key={row.id} type="button" data-panel-object-row={row.id} data-mobile-row-id={row.id} onClick={() => onNavigate(route, { objectId: row.id })}><span><b>{row.primary}</b><small>{row.secondary}</small></span><strong>{row.trailing}</strong><ChevronRight size={16} aria-hidden="true" /></button>) : <p className="ref-empty">当前筛选没有可核实的对象</p>}</section>{model.tables.slice(1).filter((tbl) => tbl.rows.length > 0).map((tbl, tblIdx) => <section key={tbl.title} className="ref-card ref-secondary-table" data-mobile-secondary-table={tblIdx}><h2>{tbl.title}</h2>{tbl.rows.slice(0, 30).map((row, rowIdx) => <div key={rowIdx} className="ref-secondary-rows">{tbl.columns.map((col) => { const val = String(row[col.key] ?? ""); return val ? <div key={col.key} className="ref-fact-row"><small>{col.label}</small><b>{val}</b></div> : null; })}</div>)}</section>)}<RouteEvidenceBoundary route={route} model={model} onNavigate={onNavigate} surface="mobile" />{visibleRows.length === 0 ? <EvidenceWorkspace route={route} model={model} /> : null}</div></div></main>;
}
