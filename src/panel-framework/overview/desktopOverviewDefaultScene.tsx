...
export function buildDefaultDesktopScene(
  ...
  const terminalRanking = <TerminalRanking key="compact-terminals" title="终端排行" subtitle="按总流量排序" module="terminal-ranking" tone="trust" trust={trust} headers={["设备", "IP", "流量", "状态"]} rows={compactRows(desktopTerminalRows(snapshot), isFleet ? 4 : 6)} minRows={0} />;
  return {
    main: [
    ...
    side: [
      <Module key="compact-interface" title="接口状态" subtitle="转发面 / 承载" module="normal-interface-boundary" tone="trust" trust={trust} headers={["对象", "当前", "最近", "边界"]} rows={interfaceRowsCompact} minRows={0} collapsed={isFleet} />,
      <Module key="compact-resource" title="资源" subtitle={isFleet ? "接口排行 / 阈值" : "当前 / 阈值"} module="resource-threshold" tone={state.facts.resource.level} trust={trust} headers={["项", "阈值", "持续", "峰值"]} rows={compactRows(resourceRows(state), 3)} minRows={0} collapsed={isFleet} />,
      <Module key="compact-collection" title={isFleet ? "采集可信度" : "采集 / 快照"} subtitle="REST / SSH / 成功" module="normal-collection-channel" tone={state.facts.collection.level} trust={trust} headers={["对象", "当前", "依据"]} rows={collectionRowsCompact} minRows={0} collapsed={isFleet} />,
    ],
    bottom: [
