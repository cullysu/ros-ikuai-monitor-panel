...
(function() {
  ...
  function snapshotCredibilityOf(snapshot) {
    ...
    if (meta.clientEvidenceBoundary || meta.realtimeError || meta.slowRestError || meta.staticError || meta.connectionDetailError || meta.connectionProtocolError || hasEndpointFailures) return "cache";
    return "realtime";
  }
  function hasWanCollection(snapshot) {
  ...
  function isDefaultRouteCandidate(route, impliedByCollection) {
  ...
  function defaultRouteRows(snapshot) {
    var _a, _b;
    const explicit = (_a = snapshot.routes) == null ? void 0 : _a.defaultRoutes;
    const rows2 = Array.isArray(explicit) ? explicit.filter((route) => isDefaultRouteCandidate(route, true)) : Array.isArray((_b = snapshot.routes) == null ? void 0 : _b.items) ? snapshot.routes.items.filter((route) => isDefaultRouteCandidate(route, false)) : [];
    return rows2.slice().sort((left, right) => {
  ...
  function defaultRouteRawSummary(route) {
    if (!route) return "";
  ...
  function defaultRouteBusinessSummary(route) {
    if (!route) return "";
    const gateway = normalize(route.gateway || route.gatewayStatus, "-");
    const distance = route.distance ?? "-";
    const state2 = route.disabled ? "已禁用" : route.active ? "已启用" : "待确认";
  ...
  function businessEvidenceState(snapshot) {
    const wanObserved = hasWanCollection(snapshot);
    const interfacesObserved = Array.isArray(snapshot.interfaces);
    const wanUnknown = wanObserved ? wanRows$3(snapshot).filter((row) => row.running !== true && row.running !== false).length : 0;
    const interfacesUnknown = interfacesObserved ? snapshot.interfaces.filter((row) => (row == null ? void 0 : row.running) !== true && (row == null ? void 0 : row.running) !== false).length : 0;
    const reasons = [];
  ...
  function collectionState(snapshot, freshness, failures = failedEndpointSummary(snapshot)) {
    ...
    const credibility = noSnapshot ? "unavailable" : channelDegraded || dataStale || freshness.credibility === "cache" ? "cache" : "realtime";
    const dataText = noSnapshot ? "无业务快照，业务数据不展示" : businessEvidence.incomplete ? businessEvidence.text : transportDegraded || failures.count > 0 ? "采集证据不完整" : dataStale ? `业务快照年龄 ${freshness.text}` : latestBusinessSuccessTime(snapshot) ? `数据层最后成功采样 ${shortTimestamp(latestBusinessSuccessTime(snapshot))}` : "成功时间未记录";
    ...
    const label = noSnapshot ? "快照缺失" : businessEvidence.incomplete ? "证据不完整" : channelDegraded ? "通道需复核" : dataStale ? "数据陈旧" : "采集可用";
    return {
      level,
      label,
      credibility,
  ...
  function routeState(snapshot, freshness) {
    ...
    const active = Boolean((selected == null ? void 0 : selected.active) === true && selected.disabled !== true);
    const base2 = { rawSummary, selected, verified: false, candidates: candidates.length };
    if (isSnapshotUnavailable(snapshot)) return { ...base2, label: "不可判定", text: "缺少当前路由快照", level: "warn" };
    ...
    if (!selected) return { ...base2, label: "待确认", text: "默认路由事实未采集", level: "warn" };
    if (!active) return { ...base2, label: "默认路由待确认", text: businessSummary || "默认路由事实未采集", level: "warn" };
    return { ...base2, verified: true, label: "活动默认路由", text: businessSummary, level: "ok" };
  }
  function resourceState(snapshot) {
    ...
    const level = !available ? "missing" : danger ? "danger" : warn || !complete ? "warn" : "ok";
    const summaryText = metrics.map((metric) => `${metric.label} ${metric.value === null ? "未记录" : formatPercent(metric.value)}`).join(" / ");
  ...
  function wanState(snapshot) {
    ...
    const rows2 = available ? wanRows$3(snapshot) : [];
    const total = rows2.length;
    const online = rows2.filter((row) => row.running === true).length;
    const offline = rows2.filter((row) => row.running === false).length;
    const unknown = Math.max(0, total - online - offline);
    const allOffline = total > 0 && offline === total && unknown === 0;
    const label = !available ? "WAN 未采集" : total === 0 ? "未发现 WAN 对象" : allOffline ? "WAN 全离线" : unknown > 0 ? "WAN 状态未完整" : offline > 0 ? "WAN 部分离线" : "WAN 可用";
    const text2 = !available ? "WAN 对象未采集" : total === 0 ? "已采集 · 0 个 WAN 对象" : `${formatNumber(online)}/${formatNumber(total)} 运行 · ${formatNumber(offline)} 离线${unknown ? ` · ${formatNumber(unknown)} 未知` : ""}`;
    return { available, total, online, offline, unknown, allOffline, label, text: text2 };
  }
  function interfaceState(snapshot) {
    const available = !isSnapshotUnavailable(snapshot) && Array.isArray(snapshot.interfaces);
    const rows2 = available ? snapshot.interfaces : [];
    const onlineRows = rows2.filter((row) => (row == null ? void 0 : row.running) === true);
    ...
    const unknown = Math.max(0, rows2.length - online - down);
    const downNames = downRows.map((row) => row.name || row.interface || "").filter(Boolean);
    const label = !available ? "接口未采集" : rows2.length === 0 ? "未发现接口对象" : unknown > 0 ? "接口状态未完整" : down > 0 ? "接口部分未运行" : "接口在线";
    const text2 = !available ? "接口对象未采集" : rows2.length === 0 ? "已采集 · 0 个接口对象" : down > 0 || unknown > 0 ? `${formatNumber(online)}/${formatNumber(rows2.length)} 运行 · ${formatNumber(down)} down${unknown ? ` · ${formatNumber(unknown)} 未知` : ""}${down ? ` · ${compactListText(downNames, 3) || "未列出"}` : ""}` : `${formatNumber(online)}/${formatNumber(rows2.length)} 运行`;
    return { available, total: available ? rows2.length : 0, online, down, unknown, downNames, label, text: text2 };
  }
  ...
  function buildVerdict(key, facts) {
    ...
    const summary = key === "no-snapshot" ? `无业务快照，业务数据不展示 / 失败端点 ${facts.collection.failedEndpointText}` : key === "resource-full" ? facts.resource.summaryText : key === "interfaces-down" ? `${formatNumber(facts.interfaces.down)} 个接口 down / 转发面优先` : key === "all-offline" ? `${facts.wan.label} / ${facts.route.label}` : key === "collection-down" ? `${facts.collection.channelText} / ${facts.collection.dataText}` : `${facts.wan.label} / ${facts.resource.summaryText}`;
    const detail = key === "no-snapshot" ? "设备当前不可达" : key === "resource-full" ? facts.resource.summaryText : key === "interfaces-down" ? compactListText(facts.interfaces.downNames, 4) || "涉及接口未列出" : key === "all-offline" ? `${formatNumber(facts.wan.offline)} 条离线线路` : key === "collection-down" ? "REST / SSH / 对象状态 / 失败端点" : facts.collection.channelText;
  ...
  const Cable = createLucideIcon("cable", __iconNode$y);
  /**
   * @license lucide-react v1.24.0 - ISC
   *
   * This source code is licensed under the ISC license.
  ...
  const ChartNoAxesCombined = createLucideIcon("chart-no-axes-combined", __iconNode$x);
  /**
   * @license lucide-react v1.24.0 - ISC
   *
   * This source code is licensed under the ISC license.
  ...
  const FileClock = createLucideIcon("file-clock", __iconNode$m);
  /**
   * @license lucide-react v1.24.0 - ISC
   *
   * This source code is licensed under the ISC license.
  ...
  const ListFilter = createLucideIcon("list-filter", __iconNode$i);
  /**
   * @license lucide-react v1.24.0 - ISC
   *
   * This source code is licensed under the ISC license.
  ...
  const Router = createLucideIcon("router", __iconNode$b);
  /**
   * @license lucide-react v1.24.0 - ISC
   *
   * This source code is licensed under the ISC license.
  ...
  const ScanSearch = createLucideIcon("scan-search", __iconNode$a);
  /**
   * @license lucide-react v1.24.0 - ISC
   *
   * This source code is licensed under the ISC license.
  ...
  function rawIdentityString(...values) {
  ...
  function canonicalRawIdentity(value) {
  ...
  function panelObjectIdentityPartsForRaw(route, table2, row) {
    ...
    if (route === "interfaces") {
    ...
    } else if (route === "dhcp" && table2 === "地址租约") {
    ...
    } else if (route === "balance" && table2 === "默认路由") {
    ...
    } else if (route === "security" && table2 === "安全告警") {
      ...
      parts = [table2, JSON.stringify(canonicalRawIdentity(row))];
    }
    return parts.some(Boolean) ? parts : [table2, JSON.stringify(canonicalRawIdentity(row))];
  }
  ...
  function panelObjectIdForValues(route, table2, values, identityParts) {
    const identity = panelObjectIdentity(route, table2, values);
    const parts = (identityParts == null ? void 0 : identityParts.some((part) => normalizePart(part))) ? identityParts : identity.parts;
  ...
  function buildResourceInstrument(snapshot, state2, risk) {
    ...
    return {
      ...
      metrics,
      points,
      accessibleSummary: metrics.map((metric) => metric.value === null ? `${metric.label} 未记录，策略阈值 ${metric.threshold}%` : `${metric.label} ${Math.round(metric.value)}%，策略阈值 ${metric.threshold}%`).join("；")
    };
  }
  ...
  function evidenceMode$1(snapshot, state2) {
    ...
    const meta = snapshot.meta || {};
    if (meta.clientEvidenceBoundary || meta.realtimeError || meta.slowRestError || meta.staticError || meta.connectionDetailError || meta.connectionProtocolError || state2.facts.collection.channelDegraded || state2.facts.failures.count > 0 || state2.facts.freshness.stale || state2.facts.freshness.history || state2.facts.freshness.credibility === "cache") return "historical";
    return "current";
  }
  ...
  function riskOf(mode, state2, route) {
    ...
    if (state2.facts.wan.allOffline) return "wan";
  ...
  function factsFor(snapshot, state2, mode, risk, route) {
    ...
    if (risk === "resource") {
      ...
      if (state2.facts.resource.cpu !== null) observedResources.push({ value: state2.facts.resource.cpu, threshold: CPU_THRESHOLD });
      if (state2.facts.resource.memory !== null) observedResources.push({ value: state2.facts.resource.memory, threshold: MEMORY_THRESHOLD });
      if (state2.facts.resource.disk !== null) observedResources.push({ value: state2.facts.resource.disk, threshold: DISK_THRESHOLD });
    ...
    if (state2.scale === "fleet") {
      const runningInterfaces = state2.facts.interfaces.online;
      return [
  ...
  function verdictFor(state2, mode, risk, route) {
    ...
    if (risk === "collection") {
      return { label: "证据已降级", title: "当前采集状态不可确认", summary: "仅保留上次成功记录；不代表当前业务。", tone: "warn" };
    }
  ...
  function priorityObjectsFor(snapshot, state2, risk) {
    ...
    if (risk === "resource") {
      const samples = resourceSampleStats(snapshot);
      const observedResources = [];
      if (state2.facts.resource.cpu !== null) observedResources.push({ key: "cpu", label: "CPU", value: state2.facts.resource.cpu, threshold: CPU_THRESHOLD });
      if (state2.facts.resource.memory !== null) observedResources.push({ key: "memory", label: "内存", value: state2.facts.resource.memory, threshold: MEMORY_THRESHOLD });
      if (state2.facts.resource.disk !== null) observedResources.push({ key: "disk", label: "磁盘", value: state2.facts.resource.disk, threshold: DISK_THRESHOLD });
      ...
      return {
        ...
        rows: [{
          ...
          reason: state2.facts.resource.complete ? samples.observed ? "检查连接压力、接口吞吐与原始采样" : "检查资源对象与采集完整性" : `已观测 ${state2.facts.resource.observed}/3；缺失项不按零处理`,
          tone: "danger",
          route: "trafficLoad",
          ...
          attributes: [
            ...
            { label: "资源观测", value: `${state2.facts.resource.observed} / 3` },
    ...
    if (risk === "route") {
      return {
        ...
        rows: [{
          ...
          attributes: [
            { label: "默认路由记录", value: `${state2.facts.route.candidates} 条` },
  ...
  function buildOverviewEvidenceModel(snapshot, state2) {
    ...
    const route = state2.facts.route.verified ? state2.facts.route.selected : null;
  ...
  function MobileResourcePressure({ resource }) {
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "mp-resource", "data-mobile-resource-signal": resource.status, "aria-labelledby": "mp-resource-title", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mp-resource-metrics", "aria-label": resource.accessibleSummary, children: resource.metrics.map((metric) => {
        ...
        const rounded = observed ? Math.round(metric.value) : null;
        const value = observed ? Math.max(0, Math.min(100, metric.value)) : 0;
        return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: observed ? "" : "is-missing", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "span",
            {
              ...
              role: observed ? "meter" : void 0,
              "aria-label": observed ? `${metric.label} ${rounded}%，策略阈值 ${metric.threshold}%` : `${metric.label} 未记录，策略阈值 ${metric.threshold}%`,
              "aria-valuemin": observed ? 0 : void 0,
              "aria-valuemax": observed ? 100 : void 0,
              "aria-valuenow": rounded ?? void 0,
              children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: rounded === null ? "未记录" : `${rounded}%` }),
      ...
      ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "mp-resource-pending", children: currentComplete ? "只有当前完整资源采样；至少两个带时间样本后才绘制趋势。" : "当前仅取得部分资源采样；缺失项不按零处理，完整时间序列到达前不绘图。" }),
  ...
  function MobilePatrolScreen({
    ...
    const runningInterfaces = state2.facts.interfaces.available ? state2.facts.interfaces.online : null;
    reactExports.useEffect(() => {
      var _a2;
  ...
  const MOBILE_PANEL_QUERY = "(max-width: 1365px)";
  ...
  function routeStatus(evidence, state2, route) {
    ...
    if (!route) return {
      key: "route",
      label: "默认路由",
      value: state2.facts.wan.allOffline ? "无活动记录" : "无法核实",
      note: "未发现 active=true 且未停用的默认路由",
      tone: state2.facts.wan.allOffline ? "danger" : "warn"
    };
  ...
  function operationalRows(snapshot, state2) {
    ...
    return [
      ...
      },
      {
        ...
        category: "设备资源",
        object: "CPU / 内存 / 磁盘",
        state: state2.facts.resource.summaryText,
        evidence: !state2.facts.resource.available ? "资源采样不可用" : state2.facts.resource.complete ? "阈值 85% / 85% / 90%" : `已观测 ${state2.facts.resource.observed}/3；缺失项不按零处理`,
  ...
  function interfaceEvidence$1(row) {
  ...
  function objectRows$1(snapshot) {
    ...
    const interfaces = interfaceRows(snapshot).slice(0, remaining).map((row, index) => ({
      ...
      state: row.disabled === true ? "已停用" : row.running === false ? "未运行" : row.running === true ? "运行" : "状态未记录",
      evidence: interfaceEvidence$1(row),
  ...
  function buildDesktopOverviewModel(snapshot, state2) {
    ...
    return {
      ...
      objectRows: evidence.evidenceMode === "current" ? objectRows$1(snapshot) : [],
      activeRoute: route
    };
  }
  ...
  function usePanelRoute() {
    ...
    const navigate = reactExports.useCallback((next, options = {}) => {
      ...
      if (objectId) state2.panelObject = objectId;
      else delete state2.panelObject;
  ...
  function hasObservedOverviewValue(value) {
    ...
    return stringFields.some((key) => stringValue$1(value[key]) !== "") || numberFields.some((key) => finiteNumber(value[key]) !== null);
  }
  ...
  function channelTest(value) {
    ...
    return {
      ok: source.ok === true,
      error: stringValue$1(source.error),
      elapsedMs: finiteNumber(source.elapsedMs),
      ...stringValue$1(source.identity) ? { identity: stringValue$1(source.identity) } : {},
      ...finiteNumber(source.status) !== null ? { status: finiteNumber(source.status) } : {},
      ...stringValue$1(source.fingerprint) ? { fingerprint: stringValue$1(source.fingerprint) } : {},
      ...stringValue$1(source.expectedFingerprint) ? { expectedFingerprint: stringValue$1(source.expectedFingerprint) } : {},
      ...stringValue$1(source.algorithm) ? { algorithm: stringValue$1(source.algorithm) } : {},
  ...
  function routerLoginProfile(value) {
    ...
    const updatedAt = stringValue$1(value.updatedAt);
    ...
    return {
      ...
      host: stringValue$1(value.host),
      user: stringValue$1(value.user),
      sshPort: Math.round(port),
      sshHostKeyFingerprint: stringValue$1(value.sshHostKeyFingerprint),
      restScheme,
      restPort: Math.round(restPort),
      restVerifyTls: value.restVerifyTls === true,
      insecureRestConfirmed: value.insecureRestConfirmed === true,
      source: stringValue$1(value.source),
      savedId: stringValue$1(value.savedId),
      updatedAt,
  ...
  function savedLogin(value) {
    ...
    const host = stringValue$1(value.host);
    const user = stringValue$1(value.user);
    const port = finiteNumber(value.sshPort);
    const restPort = finiteNumber(value.restPort);
    const restScheme = value.restScheme === "http" ? "http" : value.restScheme === "https" ? "https" : null;
    ...
    return {
      ...
      restVerifyTls: value.restVerifyTls === true,
      insecureRestConfirmed: value.insecureRestConfirmed === true,
      label: stringValue$1(value.label) || host,
      updatedAt: stringValue$1(value.updatedAt),
      lastUsedAt: stringValue$1(value.lastUsedAt),
      lastTest: connectionTest(value.lastTest)
    };
  }
  ...
  function validatePanelSnapshot(input) {
    ...
    const status = stringValue$1(input.status).toLowerCase();
  ...
  function parseRouterLoginBootstrap(input) {
    ...
    const csrfToken = stringValue$1(input.csrfToken);
  ...
  function parseRouterLoginMutation(input) {
    ...
    return {
      ...
      warning: stringValue$1(input.warning),
  ...
  function stringList(value) {
    ...
    return source.map((item) => stringValue(item)).filter((item) => Boolean(item));
  }
  function observedSum(...values) {
    const observed = values.map((value) => numberValue(value)).filter((value) => value !== null);
    return observed.length ? observed.reduce((sum, value) => sum + value, 0) : null;
  }
  function severityFrom(...values) {
    const source = values.map((value) => {
      var _a;
      return ((_a = stringValue(value)) == null ? void 0 : _a.toLocaleLowerCase()) || "";
    }).filter(Boolean).join(" ");
  ...
  function interfaceEvidence(title, row, context) {
    const name = stringValue(row.name, row.interface, row.lineId);
    const routesRecord = record$1(context.routes);
    const routeCandidates = objectRows(routesRecord.items).length ? objectRows(routesRecord.items) : objectRows(routesRecord.defaultRoutes);
    const directRoutes = routeCandidates.filter((candidate) => isDefaultRoute(candidate) && exactGatewayMatch(stringValue(candidate.gateway), name)).map(routeEvidence);
    const attachedRoutes = objectRows(row.routes).map((candidate) => ({
      ...routeEvidence(candidate),
      gateway: name || "未取得"
    }));
    const uniqueRoutes = [...directRoutes, ...attachedRoutes].filter((candidate, index, items) => items.findIndex((item) => JSON.stringify(item) === JSON.stringify(candidate)) === index);
    return {
      kind: "interface",
      sourceTable: title,
  ...
  function buildRouteEvidence(title, row, context) {
    const protocol = row.static === true ? "static" : row.dynamic === true ? "dynamic" : stringValue(row.protocol);
  ...
  function terminalEvidence(title, row, context) {
    var _a;
    const ip = stringValue(row.ip, row.address);
    const mac = ((_a = stringValue(row.mac, row.macAddress)) == null ? void 0 : _a.toLocaleLowerCase()) || null;
    const dhcp = record$1(context.dhcp);
    const lease = objectRows(dhcp.leases).find((candidate) => {
      var _a2;
      const candidateIp = stringValue(candidate.address, candidate.ip);
      const candidateMac = ((_a2 = stringValue(candidate.mac, candidate.macAddress)) == null ? void 0 : _a2.toLocaleLowerCase()) || null;
      return Boolean(ip && candidateIp === ip || mac && candidateMac === mac);
    });
    const arpRoot = Array.isArray(context.arp) ? {} : record$1(context.arp);
    ...
    const hasConnectionEvidence = [row.connections, row.downRate, row.upRate, row.sessionBytes].some((value) => numberValue(value) !== null);
    ...
    return {
      kind: "terminal",
      sourceTable: title,
      hostname: stringValue(row.displayName, row.hostname, row.name, lease == null ? void 0 : lease.hostname, lease == null ? void 0 : lease.hostName),
      ip,
      mac: stringValue(row.mac, row.macAddress),
      status: stringValue(row.status),
      online: booleanValue(row.online),
      lastSeen: stringValue(row.lastSeen, lease == null ? void 0 : lease.lastSeen),
      interfaceName: stringValue(row.interface, arp == null ? void 0 : arp.interface),
      dhcpStatus: stringValue(lease == null ? void 0 : lease.status),
      dhcpServer: stringValue(lease == null ? void 0 : lease.server),
      arpStatus: stringValue(arp == null ? void 0 : arp.status),
  ...
  function logEvidence(title, row, context) {
    const time = stringValue(row.time, row.lastConfirmed);
    const sourceRows2 = objectRows(record$1(context.logs).all);
    const ordered = sourceRows2.map((item, sourceIndex) => {
    ...
    const neighbors = currentIndex < 0 ? [] : ordered.map((candidate, index) => ({ ...candidate, index })).filter((candidate) => candidate.item !== row).sort((left, right) => Math.abs(left.index - currentIndex) - Math.abs(right.index - currentIndex) || left.index - right.index).slice(0, 2).sort((left, right) => left.index - right.index).map((candidate) => {
  ...
  function securityEvidence(title, row) {
  ...
  function dnsEvidence(route, title, row, context) {
    const dns = record$1(context.dns);
    const objectType = route === "dns4" ? "rule" : Object.prototype.hasOwnProperty.call(row, "advertiseDns") ? "ipv6-nd" : "ipv6-dhcp";
    return {
      kind: "dns",
      sourceTable: title,
      objectType,
  ...
  function resourceEvidence(title, row) {
    const values = Array.isArray(row.values) ? row.values.map((value) => numberValue(value)).filter((value) => value !== null) : [];
    return {
      kind: "resource",
      sourceTable: title,
  ...
  function connectionEvidence(title, row) {
    return {
      kind: "connection",
      sourceTable: title,
  ...
  function emptySectionRowEvidence(sourceTable = "") {
    return { kind: "generic", sourceTable, status: null };
  }
  function buildSectionRowEvidence(route, title, row, context = {}) {
  ...
  function emptySectionRowMeta(overrides = {}) {
    return {
      state: "neutral",
      attention: false,
      running: null,
  ...
  function observedTotal(...values) {
    const observed = values.map((value) => number(value)).filter((value) => value !== null);
    return observed.length ? observed.reduce((sum, value) => sum + value, 0) : null;
  }
  function rawSeverity(row, route) {
    const evidence = [row.severity, row.level, row.topics].map((value) => rawString(value).toLocaleLowerCase()).filter(Boolean).join(" ");
  ...
  function metadataFor(route, title, row) {
    ...
    const observedSeries = Array.isArray(row.values) ? row.values.map((value) => number(value)).filter((value) => value !== null) : [];
    ...
    const isDefaultRoute2 = row.default === true || destination === "0.0.0.0/0" || destination === "::/0";
    ...
    if (isDefaultRoute2) tags.add("default");
    ...
    const attention = disabled === true || rowState === "stopped" || rowState === "warning" || rowState === "error" || title === "身份告警" || title === "安全告警" || title === "采集与诊断边界";
  ...
  function table(route, title, columns, sourceRows2, map, empty, note, evidenceContext = {}) {
    return {
      title,
      columns,
      rows: sourceRows2.map(map),
      rowMeta: sourceRows2.map((row) => metadataFor(route, title, row)),
      rowEvidence: sourceRows2.map((row) => buildSectionRowEvidence(route, title, row, evidenceContext)),
      empty,
      note
    };
  }
  function evidenceMode(snapshot) {
    ...
    const meta = snapshot.meta;
    const hasEndpointFailures = [
      meta == null ? void 0 : meta.staticEndpointFailures,
      meta == null ? void 0 : meta.realtimeEndpointFailures,
      meta == null ? void 0 : meta.slowRestEndpointFailures,
      meta == null ? void 0 : meta.detailEndpointFailures
    ].some((items) => Array.isArray(items) && items.length > 0);
    if ((meta == null ? void 0 : meta.clientEvidenceBoundary) || (meta == null ? void 0 : meta.realtimeError) || (meta == null ? void 0 : meta.slowRestError) || (meta == null ? void 0 : meta.staticError) || (meta == null ? void 0 : meta.connectionDetailError) || (meta == null ? void 0 : meta.connectionProtocolError) || hasEndpointFailures) return "historical";
    return "current";
  }
  ...
  function interfaceModel(route, snapshot) {
    ...
    return {
      ...
      }), "当前快照没有接口对象", void 0, { routes: snapshot.routes })]
    };
  }
  function wanModel(route, snapshot) {
    ...
    return {
      ...
      }), "当前快照没有 WAN 对象", void 0, { routes: snapshot.routes })]
    };
  }
  function routeModel(route, snapshot) {
    ...
    return {
      ...
      }), "当前快照没有路由记录", void 0, { interfaces: snapshot.interfaces, wan: snapshot.wan })]
    };
  }
  function balanceModel(route, snapshot) {
    ...
    return {
      ...
      tables: [
        table(route, "默认路由", [{ key: "gateway", label: "网关" }, { key: "table", label: "路由表" }, { key: "distance", label: "距离" }, { key: "status", label: "状态" }], defaults, (item) => ({ gateway: text(item.gateway), table: text(item.table), distance: text(item.distance), status: item.active === true ? "活动" : item.active === false ? "非活动" : "未确认" }), "未取得默认路由", void 0, { interfaces: snapshot.interfaces, wan: snapshot.wan }),
  ...
  function terminalModel(route, snapshot) {
    ...
    return {
      ...
      tables: [table(route, "终端对象", [
        ...
        traffic: `${rate(item.downRate)} / ${rate(item.upRate)}`,
        _mac: text(item.mac, "")
      }), "当前快照没有终端记录", void 0, { dhcp: snapshot.dhcp, arp: snapshot.arp })]
    };
  }
  ...
  function arpModel(route, snapshot) {
    ...
    return {
      ...
      tables: [
        ...
        table(route, "ARP 对象", [{ key: "address", label: "IP" }, { key: "mac", label: "MAC" }, { key: "status", label: "状态" }, { key: "interface", label: "接口" }], items, (item) => ({ address: text(item.ip || item.address), mac: text(item.mac || item.macAddress), status: text(item.status, item.dynamic === true ? "动态" : "未确认"), interface: text(item.interface) }), "当前快照没有 ARP 记录", void 0, { dhcp: snapshot.dhcp, arp })
      ]
    };
  }
  ...
  function dnsModel(route, snapshot) {
    ...
    return {
      ...
      }, ipv6 ? "当前快照没有 IPv6 ND/DHCP 对象" : "当前快照没有 DNS 静态规则", void 0, { dns })]
    };
  }
  ...
  function logModel(route, snapshot) {
    ...
    return {
      ...
      tables: [table(route, route === "serviceLogs" ? "分类日志" : "最近日志", [{ key: "time", label: "时间" }, { key: "topics", label: "主题" }, { key: "message", label: "内容" }], grouped, (item) => ({ time: text(item.time), topics: route === "serviceLogs" ? `${text(item.group)} · ${text(item.topics)}` : text(item.topics), message: text(item.message) }), "当前快照没有日志记录", void 0, { logs })]
    };
  }
  ...
  function rowsFromModel(route, model) {
    ...
    model.tables.forEach((table2) => {
      table2.rows.forEach((values, index) => {
        var _a, _b;
        const meta = ((_a = table2.rowMeta) == null ? void 0 : _a[index]) || emptySectionRowMeta();
        const evidence = ((_b = table2.rowEvidence) == null ? void 0 : _b[index]) || emptySectionRowEvidence(table2.title);
        ...
        const baseId = panelObjectIdForValues(route, table2.title, values, meta.identityParts);
        const signature = JSON.stringify(canonicalValue({ table: table2.title, values, meta, evidence }));
        candidates.push({
          baseId,
          signature,
          table: table2.title,
          ...
          searchText: [
            ...
            secondary,
            trailing,
            ...table2.columns.map((column) => values[column.key] || "")
    ...
    candidates.forEach((candidate) => {
      const signatures = groups.get(candidate.baseId) || /* @__PURE__ */ new Map();
    ...
    candidates.forEach((candidate) => {
      ...
      const signatures = groups.get(candidate.baseId) || /* @__PURE__ */ new Map([[candidate.signature, 1]]);
      ...
      const id = signatures.size > 1 ? `${candidate.baseId}-${signatureHash}${collisionOrdinal}` : candidate.baseId;
      result.push({
        id,
  ...
  function useObjectHistory(route) {
    ...
    const open = reactExports.useCallback((id) => {
      const state2 = { ...window.history.state || {}, panelObject: id };
      window.history.pushState(state2, "", objectUrl(id));
      window.dispatchEvent(new PopStateEvent("popstate", { state: state2 }));
    }, []);
    const replace = reactExports.useCallback((id) => {
      const state2 = { ...window.history.state || {} };
      if (id) state2.panelObject = id;
      else delete state2.panelObject;
      window.history.replaceState(state2, "", objectUrl(id));
      window.dispatchEvent(new PopStateEvent("popstate", { state: state2 }));
    }, []);
    const close = reactExports.useCallback(() => {
      var _a;
      if (((_a = window.history.state) == null ? void 0 : _a.panelObject) === selectedId) {
        window.history.back();
        return;
      }
      replace(null);
  ...
  const RUNNING_FILTER = filter("running", "运行", (row) => (row.meta.running === true || row.meta.state === "running" || row.meta.state === "online" || row.meta.state === "bound") && !row.meta.attention);
  const NAME_ASC = sort("name-asc", "名称正序", (left, right) => compareText(left.primary, right.primary));
  const ATTENTION_FIRST = sort("attention-first", "异常优先", (left, right) => Number(right.meta.attention) - Number(left.meta.attention) || compareText(left.primary, right.primary));
  const TRAFFIC_DESC = sort("traffic-desc", "流量从高到低", (left, right) => compareNumber(left.meta.trafficBps, right.meta.trafficBps, "desc") || compareText(left.primary, right.primary));
  const STATUS_FIRST = sort("status-first", "状态优先", (left, right) => Number(right.meta.attention) - Number(left.meta.attention) || compareText(left.meta.state, right.meta.state) || compareText(left.primary, right.primary));
  ...
  const DOMAIN = {
    ...
    routes: {
      ...
      sorts: [
        sort("active-first", "活动优先", (left, right) => Number(right.meta.active === true && right.meta.disabled !== true) - Number(left.meta.active === true && left.meta.disabled !== true) || compareNumber(left.meta.distance, right.meta.distance, "asc") || compareText(left.primary, right.primary)),
        sort("distance-asc", "距离从低到高", (left, right) => compareNumber(left.meta.distance, right.meta.distance, "asc") || compareText(left.primary, right.primary)),
        NAME_ASC
      ]
    },
    ...
    security: {
      ...
      sorts: [
        sort("risk-first", "告警优先", (left, right) => Number(hasTag(right, "security-alert")) - Number(hasTag(left, "security-alert")) || compareNumber(left.meta.ruleOrder, right.meta.ruleOrder, "asc") || compareText(left.primary, right.primary)),
  ...
  function displayValue(value, fallback = "未取得") {
    if (typeof value === "number" && Number.isFinite(value)) return String(value);
    if (typeof value === "string" && value.trim()) return value.trim();
    return fallback;
  }
  function displayState(value, fallback = "未确认") {
  ...
  function displayList(values, fallback = "未取得") {
    return values.length ? values.join("、") : fallback;
  }
  function observedLabel(value, truthy, falsy) {
    return value === true ? truthy : value === false ? falsy : "未确认";
  }
  function EvidenceBoundary({ model }) {
    const label = model.evidenceMode === "current" ? "当前证据" : model.evidenceMode === "historical" ? "历史证据" : "证据不可用";
    const Icon2 = model.evidenceMode === "current" ? ShieldCheck : model.evidenceMode === "historical" ? TriangleAlert : CircleAlert;
    const absolute = formatRfc3339Local(model.observedAt);
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: `mdi-evidence is-${model.evidenceMode}`, "aria-label": "证据边界", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(Icon2, { "aria-hidden": "true", size: 16 }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("b", { children: label }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: model.status })
      ] }),
      absolute ? /* @__PURE__ */ jsxRuntimeExports.jsxs("time", { dateTime: model.observedAt || void 0, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Clock3, { "aria-hidden": "true", size: 13 }),
        absolute
      ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx("em", { children: "成功时间未记录" })
    ] });
  }
  function InspectorSection({
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: `mdi-section is-${tone}`, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: title }),
        note ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: note }) : null
      ] }),
      children
    ] });
  }
  function InspectorFacts({ facts }) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mdi-facts", children: facts.map((fact2) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `is-${fact2.tone || "neutral"}`, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: fact2.label }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("b", { children: fact2.value }),
      fact2.note ? /* @__PURE__ */ jsxRuntimeExports.jsx("em", { children: fact2.note }) : null
    ] }, fact2.label)) });
  }
  function InspectorReadings({
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mdi-readings", children: [left, right].map((item) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `is-${item.tone || "neutral"}`, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: item.label }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("b", { children: item.value }),
      item.note ? /* @__PURE__ */ jsxRuntimeExports.jsx("em", { children: item.note }) : null
    ] }, item.label)) });
  ...
  function InspectorRelations({
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mdi-relations", children: rows2.map((row, index) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `is-${row.tone || "neutral"}`, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("b", { children: row.primary }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: row.secondary })
      ] }),
      row.status ? /* @__PURE__ */ jsxRuntimeExports.jsx("em", { children: row.status }) : null
    ] }, `${row.primary}-${row.secondary}-${index}`)) });
  }
  function InspectorMessage({
    children,
    tone = "neutral"
  }) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `mdi-message is-${tone}`, children });
  }
  function InspectorDisclosure({
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("details", { className: "mdi-disclosure", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("summary", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("b", { children: title }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: note })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(ChevronDown, { "aria-hidden": "true", size: 17 })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorFacts, { facts })
    ] });
  }
  function securitySeverityTone(evidence) {
  ...
  function SecurityInspector({
    ...
    if (evidence.objectType === "alert") {
      ...
      return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorSection, { title: "告警事件", tone: securitySeverityTone(evidence), children: /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorMessage, { tone: securitySeverityTone(evidence), children: displayValue(evidence.message, "没有事件正文") }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorSection, { title: "事件证据", note: absolute ? void 0 : "原始时间缺少时区，不能当作绝对事件顺序", children: /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorFacts, { facts: [
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorSection, { title: "规则判据", tone: evidence.disabled === true ? "warn" : "neutral", children: /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorFacts, { facts: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorSection, { title: "匹配条件", note: "空值表示快照没有提供，不解释为任意匹配", children: /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorFacts, { facts: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorSection, { title: "计数器", note: hasCounter ? "只表示本次快照中的累计计数" : "本次快照未取得命中计数", children: /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorFacts, { facts: [
  ...
  function dnsObjectLabel(type) {
    if (type === "rule") return "静态规则";
    if (type === "ipv6-nd") return "IPv6 ND";
    return "DHCPv6 客户端";
  }
  function DnsInspector({
    row
  }) {
    const evidence = row.evidence;
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorSection, { title: dnsObjectLabel(evidence.objectType), children: /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorFacts, { facts: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorSection, { title: "DNS 配置边界", note: "配置存在不等于解析成功；未执行外部 DNS 可达性推断", children: /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorFacts, { facts: [
      ...
      evidence.objectType !== "rule" ? /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorSection, { title: "IPv6 发布与路由", children: /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorFacts, { facts: [
  ...
  function InterfaceRelationship({ evidence }) {
    const activeRoutes = evidence.defaultRoutes.filter((route) => route.active === true && route.disabled !== true).length;
    const routeSummary = evidence.defaultRouteRelation === "direct" ? `${evidence.defaultRoutes.length} 条直接关联 · ${activeRoutes} 条活动记录` : "未取得直接关联";
    const relationRows = evidence.defaultRoutes.map((route) => ({
      ...
      tone: route.disabled === true || route.active === false ? "warn" : "neutral"
    }));
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(
      InspectorSection,
      {
        ...
        children: [
          ...
          relationRows.length ? /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorRelations, { rows: relationRows }) : null
        ]
      }
    );
  }
  function InterfaceInspector({
    ...
    const hasQuality = [evidence.dropTotal, evidence.errorTotal, evidence.lossRate, evidence.errorRate].some((value) => value !== null);
    const readings = /* @__PURE__ */ jsxRuntimeExports.jsxs(
      InspectorSection,
      {
        title: model.evidenceMode === "current" ? "当前读数" : "历史读数",
        note: model.evidenceMode === "historical" ? "来自最近成功快照，不代表当前" : void 0,
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            InspectorReadings,
            {
          ...
          hasCounters ? /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorFacts, { facts: [
          ...
          ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorMessage, { children: "本次快照未采集累计字节与包计数。" })
        ]
      }
    );
    const relationship = /* @__PURE__ */ jsxRuntimeExports.jsx(InterfaceRelationship, { evidence });
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorSection, { title: down ? "影响判据" : "接口状态", tone: down ? "danger" : "neutral", children: /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorFacts, { facts: [
        {
          label: "运行标志",
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        InspectorSection,
        {
          ...
          children: hasQuality ? /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorFacts, { facts: [
          ...
          ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorMessage, { children: "本次快照没有可计算的丢包、错误或质量样本。" })
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        InspectorDisclosure,
  ...
  function RouteInspector({
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorSection, { title: "活动判据", tone: verified ? "neutral" : evidence.active === false || evidence.disabled === true ? "warn" : "neutral", children: /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorFacts, { facts: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorSection, { title: "路径", children: /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorFacts, { facts: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        InspectorSection,
        {
          ...
          tone: (related == null ? void 0 : related.running) === false || (related == null ? void 0 : related.disabled) === true ? "danger" : "neutral",
          children: related ? /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorFacts, { facts: [
            ...
            hasInterfaceRates ? /* @__PURE__ */ jsxRuntimeExports.jsx(
              InspectorReadings,
              {
          ...
          ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorMessage, { children: "没有可验证的关联接口；未使用名称相似或首行对象进行猜测。" })
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorSection, { title: "来源与标记", children: /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorFacts, { facts: [
  ...
  function TerminalInspector({
    ...
    const hasAccessEvidence = [evidence.dhcpStatus, evidence.dhcpServer, evidence.arpStatus].some((value) => Boolean(value));
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorSection, { title: "身份依据", note: "来源分开记录；没有来源不等于终端离线", children: /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorFacts, { facts: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs(InspectorSection, { title: historical ? "历史负载" : "当前负载", note: historical ? "来自最近成功快照" : void 0, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          InspectorReadings,
          {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorSection, { title: "接入关系", children: /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorFacts, { facts: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorSection, { title: "DHCP / ARP 证据", children: hasAccessEvidence ? /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorFacts, { facts: [
      ...
      ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorMessage, { children: "本次快照没有可关联的 DHCP 或 ARP 记录。" }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        InspectorDisclosure,
        {
          title: "原始对象身份",
  ...
  function LogInspector({
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorSection, { title: "事件记录", tone: risky ? "danger" : evidence.severity === "warning" ? "warn" : "neutral", children: /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorMessage, { tone: risky ? "danger" : evidence.severity === "warning" ? "warn" : "neutral", children: displayValue(evidence.message, "没有事件正文") }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorSection, { title: "时间与级别", note: absolute ? "时间包含明确时区并按查看者本地时间显示" : "原始时间缺少时区，未作为绝对时间排序", children: /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorFacts, { facts: [
        { label: absolute ? "绝对时间" : "原始时间", value: absolute || displayValue(evidence.time) },
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorSection, { title: "相邻事件", note: "按有效 RFC 3339 时间排序；只显示当前记录附近的事件", children: evidence.neighbors.length ? /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorRelations, { rows: evidence.neighbors.map((neighbor) => ({
        ...
        tone: neighbor.severity === "critical" || neighbor.severity === "error" ? "danger" : neighbor.severity === "warning" ? "warn" : "neutral"
      })) }) : /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorMessage, { children: "当前快照没有可定位的相邻日志记录。" }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        InspectorDisclosure,
        {
          title: "记录身份",
  ...
  function ResourceInspector({ row }) {
    const evidence = row.evidence;
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs(InspectorSection, { title: "样本摘要", note: "没有配套时间戳时不把数值连接成趋势", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          InspectorReadings,
          {
  ...
  function ConnectionInspector({ row }) {
    const evidence = row.evidence;
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorSection, { title: "连接端点", children: /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorFacts, { facts: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs(InspectorSection, { title: "当前记录", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          InspectorReadings,
          {
  ...
  function GenericInspector({ row }) {
    const facts = row.columns.slice(0, 6).map((column) => ({
      label: column.label,
      value: displayValue(row.values[column.key])
    }));
    return /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorSection, { title: "记录证据", note: "该低频对象尚未建立专用关系模型", children: /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorFacts, { facts: [...facts, { label: "对象 ID", value: row.id }] }) });
  }
  function DomainInspectorBody({
    row,
    model
  }) {
    if (row.evidence.kind === "interface") return /* @__PURE__ */ jsxRuntimeExports.jsx(InterfaceInspector, { row, model });
    if (row.evidence.kind === "route") return /* @__PURE__ */ jsxRuntimeExports.jsx(RouteInspector, { row, model });
    if (row.evidence.kind === "terminal") return /* @__PURE__ */ jsxRuntimeExports.jsx(TerminalInspector, { row, model });
    if (row.evidence.kind === "log") return /* @__PURE__ */ jsxRuntimeExports.jsx(LogInspector, { row, model });
    if (row.evidence.kind === "security") return /* @__PURE__ */ jsxRuntimeExports.jsx(SecurityInspector, { row, model });
    if (row.evidence.kind === "dns") return /* @__PURE__ */ jsxRuntimeExports.jsx(DnsInspector, { row, model });
    if (row.evidence.kind === "resource") return /* @__PURE__ */ jsxRuntimeExports.jsx(ResourceInspector, { row });
    if (row.evidence.kind === "connection") return /* @__PURE__ */ jsxRuntimeExports.jsx(ConnectionInspector, { row });
    return /* @__PURE__ */ jsxRuntimeExports.jsx(GenericInspector, { row });
  }
  function MobileDomainInspector({
    row,
    model,
    route,
    onClose,
    ...
    var _a;
    if (!row) {
      return /* @__PURE__ */ jsxRuntimeExports.jsxs("aside", { className: "mdw-inspector is-boundary", "aria-labelledby": "mdw-boundary-title", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: PANEL_ROUTES[route].shortTitle }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "证据工作区" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceBoundary, { model }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "mdi-no-object", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { id: "mdw-boundary-title", children: "当前没有可检查对象" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: ((_a = model.tables[0]) == null ? void 0 : _a.empty) || model.status }),
          model.evidenceMode === "unavailable" ? /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorMessage, { tone: "danger", children: "业务对象与业务数字已隐藏；等待新的成功快照。" }) : null
        ] })
      ] });
    }
      ...
      "aside",
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { children: [
            ...
            ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: preview ? "优先预览" : "所选对象" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: row.table })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceBoundary, { model }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "mdi-object-heading", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("small", { children: [
                PANEL_ROUTES[route].shortTitle,
                " / ",
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("b", { className: row.meta.attention ? "is-attention" : "", children: stateLabel })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mdi-domain-body", children: /* @__PURE__ */ jsxRuntimeExports.jsx(DomainInspectorBody, { row, model }) })
        ]
      }
    );
  }
  ...
  function MetricStrip({ model }) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("section", { className: "mdw-metrics", "aria-label": "领域摘要", children: model.metrics.slice(0, 3).map((metric) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `is-${metric.tone || "trust"}`, children: [
  ...
  function MobileDomainWorkspace({
    ...
    const [tablet, setTablet] = reactExports.useState(false);
    const [toolsOpen, setToolsOpen] = reactExports.useState(false);
    ...
    const previewRow = tablet && !selectedRow ? visibleRows.find((row) => row.meta.attention) || visibleRows.find((row) => row.evidence.kind === "interface" && row.evidence.defaultRouteRelation === "direct" && row.meta.running === true) || visibleRows.find((row) => row.meta.active === true && row.meta.tags.includes("default")) || visibleRows[0] || null : null;
    ...
    const controlsActive = Boolean(query || filter2 !== ((_b = definition.filters[0]) == null ? void 0 : _b.id) || sort2 !== definition.defaultSort);
    reactExports.useEffect(() => {
    ...
    reactExports.useLayoutEffect(() => {
      ...
      (_b2 = rowRefs.current.get(lastTriggerRef.current)) == null ? void 0 : _b2.focus({ preventScroll: true });
      ...
      "main",
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "mdw-header", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mdw-title-row", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "mdw-title-icon", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Icon2, { "aria-hidden": "true", size: 20 }) }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mdw-layout", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "mdw-list-pane", "aria-label": model.title + "对象列表", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mdw-list-heading", children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: controlsActive ? `从 ${allRows.length} 个对象中筛选` : model.description }),
                hasControls ? /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "button",
                  {
              ...
              hasControls && toolsOpen ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mdw-controls", id: "mdw-domain-controls", "data-domain-controls": route, children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mdw-filter-row", children: [
                  definition.filters.length > 1 ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { "aria-label": `${definition.objectLabel}筛选`, children: definition.filters.map((item) => /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "button",
                    {
                      type: "button",
                ...
                controlsActive ? /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "mdw-reset-controls", type: "button", onClick: resetControls, children: "清除筛选" }) : null
              ] }) : null,
              visibleRows.length ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mdw-object-list", children: visibleRows.map((row) => {
                const selected = (selectedRow == null ? void 0 : selectedRow.id) === row.id;
                const previewed = !selected && (previewRow == null ? void 0 : previewRow.id) === row.id;
                return /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "button",
                  {
                    type: "button",
                    ...
                    children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `mdw-row-mark is-${row.meta.state}`, "aria-hidden": "true" }),
                      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "mdw-row-copy", children: [
                        /* @__PURE__ */ jsxRuntimeExports.jsx("b", { children: row.primary }),
                        /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: row.secondary })
                      ] }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: row.meta.attention ? "mdw-row-state is-attention" : "mdw-row-state", children: row.trailing }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx(ChevronRight, { "aria-hidden": "true", size: 17 })
                    ]
                  },
                  row.id
                ...
                allRows.length ? /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", onClick: resetControls, children: "清除筛选" }) : null
              ] }),
              pageCount > 1 ? /* @__PURE__ */ jsxRuntimeExports.jsxs("nav", { className: "mdw-pagination", "aria-label": "对象分页", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { type: "button", disabled: activePage <= 1, onClick: () => {
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { type: "button", disabled: activePage >= pageCount, onClick: () => {
                  if (selectedId) replace(null);
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "mdw-domain-context", "aria-label": "领域摘要证据", children: [
  ...
  function valueOf(value, fallback = "未取得") {
    return value === null || value === void 0 || value === "" ? fallback : String(value);
  }
  function booleanOf(value, positive, negative) {
  ...
  function FactGrid({ facts }) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("dl", { className: "ddi-facts", children: facts.map((fact2) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("dt", { children: fact2.label }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("dd", { children: fact2.value })
    ] }, fact2.label)) });
  }
  function EvidenceBlock({ title, note, facts, children }) {
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "ddi-block", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: title }),
        note ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: note }) : null
      ] }),
      facts ? /* @__PURE__ */ jsxRuntimeExports.jsx(FactGrid, { facts }) : null,
      children
    ] });
  }
  function InterfaceEvidence({ row }) {
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceBlock, { title: "状态与身份", facts: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceBlock, { title: "当前吞吐与累计量", facts: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceBlock, { title: "默认出口关系", note: evidence.defaultRouteRelation === "direct" ? "仅展示与该接口明确关联的默认路由" : "当前快照没有明确对象关联", facts: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceBlock, { title: "链路质量", facts: [
  ...
  function RouteEvidence({ row }) {
    ...
    const related = evidence.relatedInterface;
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceBlock, { title: "路由判定", facts: [
        { label: "目标", value: valueOf(evidence.destination) },
        { label: "网关", value: valueOf(evidence.gateway) },
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceBlock, { title: "关联接口", note: evidence.interfaceRelation === "direct" ? "由出接口或网关精确关联" : "没有足够证据确认承载接口", facts: [
        { label: "接口", value: valueOf(related == null ? void 0 : related.name) },
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceBlock, { title: "原始说明", facts: [{ label: "备注", value: valueOf(evidence.comment, "未记录") }, { label: "对象身份", value: row.id }] })
    ] });
  }
  function TerminalEvidence({ row }) {
    ...
    const evidence = row.evidence;
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceBlock, { title: "终端身份", facts: [
        { label: "主机名", value: valueOf(evidence.hostname) },
        { label: "IP / MAC", value: `${valueOf(evidence.ip)} / ${valueOf(evidence.mac)}` },
        { label: "接入接口", value: valueOf(evidence.interfaceName) },
        { label: "身份来源", value: evidence.identitySources.join(" · ") || "未取得" }
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceBlock, { title: "在线与负载", facts: [
        { label: "在线", value: booleanOf(evidence.online, "在线", "离线") },
        { label: "最后出现", value: valueOf(evidence.lastSeen) },
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceBlock, { title: "地址证据", facts: [
        { label: "DHCP", value: valueOf(evidence.dhcpStatus) },
        { label: "DHCP 服务", value: valueOf(evidence.dhcpServer) },
        { label: "ARP", value: valueOf(evidence.arpStatus) },
        { label: "对象身份", value: row.id }
      ] })
    ] });
  }
  function LogEvidence({ row }) {
    if (row.evidence.kind !== "log") return null;
    const evidence = row.evidence;
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceBlock, { title: "事件", facts: [
        { label: "时间", value: valueOf(evidence.time) },
        { label: "级别 / 主题", value: `${evidence.severity} / ${valueOf(evidence.topics)}` },
        { label: "来源", value: valueOf(evidence.source) },
        { label: "正文", value: valueOf(evidence.message) }
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceBlock, { title: "相邻事件", note: "按带时区 RFC 3339 时间排序", children: /* @__PURE__ */ jsxRuntimeExports.jsx("ol", { className: "ddi-timeline", children: evidence.neighbors.length ? evidence.neighbors.map((item, index) => /* @__PURE__ */ jsxRuntimeExports.jsxs("li", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: item.relation === "newer" ? "较新" : "较早" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("b", { children: valueOf(item.time) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: valueOf(item.message) })
      ] }, `${item.relation}-${item.time}-${index}`)) : /* @__PURE__ */ jsxRuntimeExports.jsx("li", { className: "is-empty", children: "没有可比较的相邻事件" }) }) })
    ] });
  }
  function SecurityEvidence({ row }) {
    if (row.evidence.kind !== "security") return null;
    const evidence = row.evidence;
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceBlock, { title: evidence.objectType === "rule" ? "规则语义" : "告警语义", facts: [
        ...
        { label: "严重度 / 时间", value: `${evidence.severity} / ${valueOf(evidence.time)}` },
        { label: "影响", value: valueOf(evidence.affected) }
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceBlock, { title: "匹配与计数", facts: [
  ...
  function DnsEvidence({ row }) {
    if (row.evidence.kind !== "dns") return null;
    const evidence = row.evidence;
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceBlock, { title: "DNS 对象", facts: [
        { label: "类型", value: evidence.objectType },
        { label: "名称 / 记录", value: `${valueOf(evidence.name)} / ${valueOf(evidence.recordType)}` },
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceBlock, { title: "发布与上游", facts: [
        { label: "发布 DNS", value: evidence.publishedDns.join(" · ") || "未取得" },
        { label: "上游 DNS", value: evidence.upstreamServers.join(" · ") || "未取得" },
        { label: "DoH", value: valueOf(evidence.dohServer, "未配置") },
        { label: "DoH 证书", value: booleanOf(evidence.verifyDohCert, "验证", "不验证") },
        { label: "对端 DNS", value: booleanOf(evidence.peerDns, "采用", "不采用") },
        { label: "发布默认路由", value: booleanOf(evidence.addDefaultRoute, "是", "否") }
      ] })
    ] });
  }
  function OtherEvidence({ row }) {
    if (row.evidence.kind === "resource") {
      const values = row.evidence.values;
      return /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceBlock, { title: "资源样本", note: "没有时间戳时不连接成趋势", facts: [
        { label: "最近值", value: values.length ? `${values[values.length - 1]}%` : "未取得" },
        { label: "样本数", value: `${row.evidence.sampleCount} 个` },
        { label: "下界 / 上界", value: values.length ? `${Math.min(...values)}% / ${Math.max(...values)}%` : "未取得" },
        { label: "对象身份", value: row.id }
      ] });
    }
    if (row.evidence.kind === "connection") return /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceBlock, { title: "连接证据", facts: [
      { label: "源", value: valueOf(row.evidence.source) },
      { label: "目标", value: valueOf(row.evidence.target) },
      ...
      { label: "会话字节", value: bytesOf(row.evidence.sessionBytes) },
      { label: "对象身份", value: row.id }
    ] });
    return /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceBlock, { title: "记录证据", note: "该低频对象尚未建立专用关系模型", facts: [
      ...row.columns.slice(0, 6).map((column) => ({ label: column.label, value: valueOf(row.values[column.key]) })),
      { label: "对象身份", value: row.id }
    ] });
  }
  function DesktopDomainInspector({ row, model, pinned, onUnpin, titleRef }) {
    var _a;
    if (!row) return /* @__PURE__ */ jsxRuntimeExports.jsxs("aside", { className: "ddw-inspector is-empty", "aria-label": "对象证据", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "ddi-boundary", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("b", { children: model.evidenceMode === "current" ? "当前证据" : model.evidenceMode === "historical" ? "历史证据" : "证据不可用" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: model.updatedAt || "未记录成功时间" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "当前没有可检查对象" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: ((_a = model.tables[0]) == null ? void 0 : _a.empty) || model.status })
    ] });
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("aside", { className: "ddw-inspector", "data-desktop-object-detail": row.id, "data-domain-inspector-kind": row.evidence.kind, "aria-labelledby": "desktop-domain-title", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "ddi-boundary", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("b", { children: model.evidenceMode === "current" ? "当前证据" : model.evidenceMode === "historical" ? "历史证据" : "证据不可用" }),
          model.updatedAt || "未记录成功时间"
        ] }),
        pinned ? /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", onClick: onUnpin, children: "返回自动预览" }) : /* @__PURE__ */ jsxRuntimeExports.jsx("em", { children: "自动预览" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "ddi-heading", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: row.table }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { id: "desktop-domain-title", tabIndex: -1, ref: titleRef, children: row.primary }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: row.secondary }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("b", { className: row.meta.attention ? "is-attention" : "", children: row.trailing })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "ddi-body", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(InterfaceEvidence, { row }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(RouteEvidence, { row }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(TerminalEvidence, { row }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(LogEvidence, { row }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(SecurityEvidence, { row }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(DnsEvidence, { row }),
        !["interface", "route", "terminal", "log", "security", "dns"].includes(row.evidence.kind) ? /* @__PURE__ */ jsxRuntimeExports.jsx(OtherEvidence, { row }) : null
      ] })
    ] });
  }
  function preferredRow(rows2) {
    return rows2.find((row) => row.meta.attention) || rows2.find((row) => row.evidence.kind === "interface" && row.evidence.defaultRouteRelation === "direct" && row.meta.running === true) || rows2.find((row) => row.meta.active === true && row.meta.tags.includes("default")) || rows2[0] || null;
  }
  function comparisonValue(row) {
    if (row.evidence.kind === "interface") {
      ...
      return `${(rxRate / 1e6).toFixed(2)} / ${(txRate / 1e6).toFixed(2)} Mbps`;
    }
    ...
    if (row.evidence.kind === "connection") return row.evidence.trafficBps === null ? "流量未取得" : `${Math.round(row.evidence.trafficBps / 1e3)} Kbps`;
    return row.secondary;
  }
  function DesktopDomainWorkspace({ route, model }) {
    var _a, _b;
    const definition = domainDefinitionFor(route);
    const allRows = reactExports.useMemo(() => rowsFromModel(route, model), [model, route]);
    const { selectedId, open, replace, close } = useObjectHistory(route);
    const [query, setQuery] = reactExports.useState("");
    const [filter2, setFilter] = reactExports.useState(((_a = definition.filters[0]) == null ? void 0 : _a.id) || "all");
    const [sort2, setSort] = reactExports.useState(definition.defaultSort);
    ...
    const filtered = reactExports.useMemo(() => {
      const needle = query.trim().toLocaleLowerCase();
      const searched = allRows.filter((row) => !needle || row.searchText.includes(needle));
      return sortWorkspaceRows(filterWorkspaceRows(searched, definition, filter2), definition, sort2);
    }, [allRows, definition, filter2, query, sort2]);
    const pageSize = 25;
    const pageCount = Math.max(1, Math.ceil(filtered.length / pageSize));
    const selectedIndex = selectedId ? filtered.findIndex((row) => row.id === selectedId) : -1;
    ...
    reactExports.useLayoutEffect(() => {
      var _a2, _b2;
      if (selectedRow) {
        (_a2 = titleRef.current) == null ? void 0 : _a2.focus({ preventScroll: true });
        return;
      }
      if (lastTriggerRef.current) (_b2 = rowRefs.current.get(lastTriggerRef.current)) == null ? void 0 : _b2.focus({ preventScroll: true });
    }, [selectedRow]);
    const selectRow = (row) => {
      lastTriggerRef.current = row.id;
      open(row.id);
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "desktop-domain-workspace", "data-desktop-domain-workspace": route, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "ddw-toolbar", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "ddw-search", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Search, { "aria-hidden": "true", size: 16 }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "sr-only", children: [
            "搜索",
            definition.objectLabel
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("input", { type: "search", value: query, placeholder: definition.searchPlaceholder, onChange: (event) => {
            setQuery(event.target.value);
            setPage(1);
          } })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "ddw-filters", "aria-label": "对象筛选", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(SlidersHorizontal, { "aria-hidden": "true", size: 16 }),
          definition.filters.map((item) => /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", className: filter2 === item.id ? "is-active" : "", "aria-pressed": filter2 === item.id, onClick: () => {
            setFilter(item.id);
            setPage(1);
          }, children: item.label }, item.id))
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "ddw-sort", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "ddw-body", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "ddw-table-pane", "aria-label": `${definition.objectLabel}列表`, children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("b", { children: filtered.length }),
              " 个",
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: query || filter2 !== ((_b = definition.filters[0]) == null ? void 0 : _b.id) ? `共 ${allRows.length} 个对象` : "当前只读快照" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "ddw-table-scroll", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("table", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("thead", { children: /* @__PURE__ */ jsxRuntimeExports.jsxs("tr", { children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("th", { scope: "col", children: "对象" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("th", { scope: "col", children: "来源" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("th", { scope: "col", children: "状态" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("th", { scope: "col", children: "关键证据" })
              ] }) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("tbody", { children: visibleRows.map((row) => /* @__PURE__ */ jsxRuntimeExports.jsxs("tr", { className: row.meta.attention ? "is-attention" : "", "data-desktop-row-id": row.id, children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("td", { children: /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { type: "button", className: (selectedRow == null ? void 0 : selectedRow.id) === row.id ? "is-selected" : "", "aria-current": (selectedRow == null ? void 0 : selectedRow.id) === row.id ? "true" : void 0, ref: (node) => {
                  if (node) rowRefs.current.set(row.id, node);
                  else rowRefs.current.delete(row.id);
                }, onClick: () => selectRow(row), children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("b", { children: row.primary }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: row.secondary })
                ] }) }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("td", { children: row.table }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("td", { children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: row.meta.attention ? "is-attention" : "", children: row.trailing }) }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("td", { children: comparisonValue(row) })
              ] }, row.id)) })
            ] }),
            !visibleRows.length ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "ddw-empty", children: "没有符合当前条件的对象。" }) : null
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("footer", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { type: "button", disabled: activePage <= 1, onClick: () => setPage(Math.max(1, activePage - 1)), children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(ChevronLeft, { "aria-hidden": "true", size: 16 }),
              "上一页"
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
              "第 ",
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { type: "button", disabled: activePage >= pageCount, onClick: () => setPage(Math.min(pageCount, activePage + 1)), children: [
              "下一页",
              /* @__PURE__ */ jsxRuntimeExports.jsx(ChevronRight, { "aria-hidden": "true", size: 16 })
            ] })
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(DesktopDomainInspector, { row: inspectorRow, model, pinned: Boolean(selectedRow), onUnpin: unpin, titleRef })
      ] })
    ] });
  }
  ...
  function OperationalSectionPage({ route, snapshot, onNavigate }) {
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { id: route, className: "section panel-operational-section", "data-panel-route": route, "data-panel-route-content": route, "data-panel-evidence-mode": model.evidenceMode, children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(DesktopDomainWorkspace, { route, model }),
