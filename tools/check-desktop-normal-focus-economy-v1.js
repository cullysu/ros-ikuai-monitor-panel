...
const checks = {
  ...
  freshRuntimeIsBound: runtimeSkipped || (Boolean(runtimeCheck) && targetWidths.every((key) => geometry[key].focusHeight !== null)),
  allTargetViewportsKeepActionInFirstViewport: runtimeSkipped || targetWidths.every((key) => geometry[key].actionInFirstViewport === true),
  allTargetViewportsKeepNoOverflow: runtimeSkipped || targetWidths.every((key) => geometry[key].overflow !== null && geometry[key].overflow <= 1),
  focusTailSpaceIsBounded: runtimeSkipped || targetWidths.every((key) => geometry[key].trailingSpace !== null && geometry[key].trailingSpace <= target),
};
