...
const mobileTimeEvidencePath = path.join(root, "src", "panel-framework", "mobile-flow-ui", "overview", "MobileFlowOverview.tsx");
...
const mobileTruthPath = path.join(root, "src", "panel-framework", "mobile-flow-ui", "overview", "mobileFlowModel.ts");
...
assert.match(mobileTimeEvidenceSource, /data-mobile-flow-overview/, "Flow mobile overview must expose its dedicated presentation marker");
assert.match(mobileTimeEvidenceSource, /data-evidence-mode=\{model\.evidenceMode\}/, "Flow mobile overview must carry the shared evidence mode on its root");
assert.match(mobileTimeModelSource, /const evidenceAt = mode === "unavailable" \? null : latestBusinessSuccessTime\(snapshot\)/, "mobile evidence boundary must preserve the qualified observation timestamp from the shared evidence model");
...
assert.match(mobileTruthSource, /evidenceLabel:\s*evidence\.evidenceLabel[\s\S]{0,160}evidenceTime:\s*evidence\.evidenceTime/, "Flow evidence status must retain the shared typed evidence time");
assert.match(mobileTimeEvidenceSource, /data-evidence-mode=\{model\.evidenceMode\}/, "Flow evidence boundary must render the shared evidence mode directly");
...
assert.match(mobileTruthSource, /scene === "normal" && evidence\.evidenceMode === "current" && evidence\.traffic\?\.status === "ready"/, "Flow numeric WAN rates must refuse partial or non-current evidence");
