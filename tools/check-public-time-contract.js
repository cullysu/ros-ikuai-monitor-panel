...
const mobileTimeEvidencePath = path.join(opticalRoot, "OpticalPatrol.tsx");
const mobileTimeModelPath = path.join(opticalRoot, "buildOpticalPatrolModel.ts");
const mobileTimeSupportPath = path.join(opticalRoot, "opticalPatrolModelSupport.ts");
const mobileCollectionPath = path.join(opticalRoot, "opticalPatrolCollectionClaims.ts");
...
assert.match(mobileTimeEvidenceSource, /<time\s+dateTime=\{model\.evidence\.observedAt\}/, "mobile evidence must expose its qualified timestamp through native time semantics");
assert.match(mobileCollectionSource, /formatRfc3339LocalTime\(channel\.successAt\)/, "mobile collection evidence must format qualified timestamps through the shared contract");
assert.match(mobileTimeSupportSource, /sampleTimestamp:\s*latest\.timestamp/g, "mobile current-rate measurements must retain the atomic sample timestamp");
