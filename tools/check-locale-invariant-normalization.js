...
const mobileOwner = path.join(sourceRoot, "mobile-flow-ui");
const failures = [];
...
const workspace = path.join(mobileOwner, "workspace", "MobileFlowWorkspace.tsx");
const model = path.join(mobileOwner, "overview", "mobileFlowModel.ts");
...
requirePattern(workspace, /function normalized\(value: string\): string\s*\{\s*return value\.trim\(\)\.toLowerCase\(\);\s*\}/, "mobile-flow-workspace-must-use-toLowerCase-for-display-search-normalization");
requirePattern(workspace, /data-mobile-flow-workspace/, "mobile-flow-workspace-must-expose-its-owner-boundary");
requirePattern(model, /evidence\.routeEvidence\.activePath/, "route-verification-must-remain-derived-from-evidence");
...
const report = { pass: failures.length === 0, contract: "locale-invariant-normalization-v4-mobile-flow", implementationState: failures.length === 0 ? "blocking-green" : "blocking-red", failures };
