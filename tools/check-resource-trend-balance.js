  ...
  try {
    ...
    const expression = `(async () => {
      ...
      if (routerMobileApp && sectionName.startsWith('mobile')) {
        ...
        const metrics = Array.from(routerMobileApp.querySelectorAll('.rm-metric'));
        const evidenceRows = Array.from(routerMobileApp.querySelectorAll('.rm-evidence-list article'));
        ...
        const checks = {
          ...
          fourPrimaryMetrics: metrics.length === 4,
          trafficSourceTruthful: expectedScenario === 'no-snapshot'
            ? traffic?.getAttribute('data-router-mobile-traffic') === 'unavailable'
            : ['history', 'snapshot'].includes(traffic?.getAttribute('data-router-mobile-traffic') || ''),
          evidenceBounded: evidenceRows.length >= 1 && evidenceRows.length <= 5,
          ...
          readonlyBoundaryVisible: currentText.includes('只读监控') && currentText.includes('不会修改路由器配置'),
