...
for (const selector of ["data-supplemental-surface", "data-supplemental-request", "data-supplemental-state", "data-supplemental-evidence", "data-supplemental-source", "data-supplemental-coverage", "data-supplemental-observed-at", "data-supplemental-target", "data-supplemental-offset", "data-supplemental-page-size", "data-supplemental-total", "data-supplemental-connection-row", "data-supplemental-rate-origin", "data-supplemental-object-detail"]) {
  assert.match(desktopViewSource, new RegExp(selector), `${selector} must be exposed by the desktop presentation owner`);
}
...
const mobileReferenceSurfaceSource = require("node:fs").readFileSync(path.join(__dirname, "..", "src", "panel-framework", "mobile-reference-ui", "MobileReferenceSurface.tsx"), "utf8");
...
// The rebuilt mobile reference UI retired endpoint supplements: phone
// workspaces render bounded snapshot rows, and every endpoint/evidence
// contract lives on the desktop owner. Pin that split so supplements can
// never silently return to the phone surface.
assert.doesNotMatch(mobileOwnerSource, /MobileRouteSupplement|RouteSupplementEvidence|useRouteSupplementEvidence/, "mobile app must not mount endpoint supplements; they are desktop-only");
assert.doesNotMatch(mobileReferenceSurfaceSource, /data-supplemental-surface|RouteSupplementEvidence|useRouteSupplementEvidence/, "mobile reference workspaces must stay bounded snapshot presentations without endpoint supplements");
...
assert.match(desktopOwnerSource, /supplementOwnsDnsList/, "accepted DNS supplement must own its one visible collection");
assert.match(desktopOwnerSource, /supplementOwnsConnectionList/, "accepted connection query must own its one visible collection");
...
console.log("route supplement contract PASS: strict REST generations/409 replace, health freshness, canonical connection submit, 429/race state, history-backed detail, semantic rate origins and desktop-only presentation ownership");
