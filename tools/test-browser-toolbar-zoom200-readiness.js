...
assert.match(windowsZoomSource, /NoPatternInterfaceError[\s\S]*get_elem_interface\(control\.element_info\.element, \"LegacyIAccessible\"\)[\s\S]*DoDefaultAction\(\)/, 'Edge activation must retain a direct UIA LegacyIAccessible fallback without physical input');
