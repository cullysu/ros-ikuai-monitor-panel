...
assert.match(windowsZoomSource, /foreground_handle = int\(ctypes\.windll\.user32\.GetForegroundWindow\(\) or 0\)[\s\S]*IsWindowVisible[\s\S]*popup_windows\.sort\(/, 'Zoom popup roots must be visible and foreground/owner prioritized before bounded control lookup');
