...
assert.match(windowsZoomSource, /zoom_matches = find_direct_automation_id_matches\(\(window,\), zoom_in_automation_ids\)[\s\S]*owned_windows = owned_uia_windows\(desktop, owned_process_id, handle\)[\s\S]*zoom_matches = find_buttons/, 'Zoom in lookup must prioritize the original Edge window and refresh same-process popup roots at most once before bounded fallbacks');
