...
assert.match(windowsZoomSource, /desktop\.windows\(process=owned_process_id, visible_only=False\)/, 'Edge menu lookup must include delayed same-process UIA popup roots');
...
assert(windowsZoomSource.includes('candidate.exists(timeout=0.5) and ui_control_is_visible_and_enabled(candidate)'), 'Known Edge Settings and more controls must remain enabled and visible before invocation');
assert.match(windowsZoomSource, /for zoom_search_attempts in range\(1, 13\):[\s\S]*owned_windows = owned_uia_windows\(desktop, owned_process_id, handle\)[\s\S]*zoom_matches = find_buttons/, 'Each bounded Zoom in search must refresh same-process popup roots instead of reusing a stale snapshot');
