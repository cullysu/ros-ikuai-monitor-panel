...
assert.match(windowsZoomSource, /def invoke_owned_control[\s\S]*control_process_id != owned_process_id[\s\S]*require_owned_foreground_process[\s\S]*getattr\(control, "invoke", None\)[\s\S]*LegacyIAccessible/, 'Edge controls must be process-bound and activated through semantic UIA patterns only');
...
assert.match(windowsZoomSource, /all_owned_windows = owned_uia_windows\(desktop, owned_process_id, handle\)[\s\S]*popup_windows = \[[\s\S]*zoom_matches = find_direct_automation_id_matches\([\s\S]*if not zoom_matches:[\s\S]*zoom_matches = find_bounded_raw_matches\(/, 'Zoom in lookup must use same-process popup roots, a bounded AutomationId lookup, and a bounded RawView fallback without probing the full original Edge tree first');
