...
assert.match(windowsZoomSource, /def invoke_owned_control[\s\S]*accepted_process_ids[\s\S]*control_process_id not in accepted_process_ids[\s\S]*require_owned_foreground_process[\s\S]*getattr\(control, "invoke", None\)[\s\S]*LegacyIAccessible/, 'Edge controls must be bound to verified Edge owner processes and activated through semantic UIA patterns only');
assert.match(windowsZoomSource, /Prefer the semantic UIA Invoke\/Select patterns[\s\S]*semantic_action[\s\S]*LegacyIAccessible/, 'Edge activation must use semantic pywinauto UIA patterns, never physical click_input');
...
assert.match(windowsZoomSource, /def owned_uia_windows\([\s\S]*EnumWindows[\s\S]*window_is_owned_by/, 'Edge menu lookup must include process-owned top-level popup HWNDs without a desktop-wide UIA scan');
assert.doesNotMatch(windowsZoomSource, /desktop\.windows\(process=owned_process_id, visible_only=False\)/, 'Edge menu lookup must not trigger a desktop-wide UIA process enumeration');
...
assert.match(windowsZoomSource, /candidate\.exists\(timeout=0\)[\s\S]*is_original_edge_shell_control\(candidate\)[\s\S]*ui_control_is_visible_and_enabled\(candidate\)/, 'Known Edge Settings and more controls must remain bound, enabled, and visible before invocation');
assert.match(windowsZoomSource, /all_owned_windows = owned_uia_windows\(desktop, owned_process_id, handle[^\)]*\)[\s\S]*popup_windows = \[[\s\S]*zoom_matches = find_direct_automation_id_matches\([\s\S]*if not zoom_matches:[\s\S]*zoom_matches = find_bounded_raw_matches\(/, 'Zoom in lookup must use bounded Edge owner-chain popup roots, a bounded AutomationId lookup, and a bounded RawView fallback without probing the full original Edge tree first');
assert.match(windowsZoomSource, /for discovery_round in range\(2\)[\s\S]*foreground_handle = int\(ctypes\.windll\.user32\.GetForegroundWindow\(\) or 0\)[\s\S]*IsWindowVisible[\s\S]*popup_windows\.sort\(/, 'Zoom popup roots must be rediscovered once with visible and foreground/owner prioritization before bounded control lookup');
assert.match(windowsZoomSource, /user32\.EnumChildWindows\(owned_window_handle, collect_child, 0\)/, 'Zoom lookup must include bounded same-process Edge child HWND menu projections without desktop-wide UIA enumeration');
assert.match(windowsZoomSource, /is_original_edge_shell_control\(candidate\)[\s\S]*ui_control_is_visible_and_enabled/, 'Settings and more must be bound to the original Edge shell HWND, never a page control with a similar label');
assert.match(windowsZoomSource, /Prefer the semantic UIA Invoke\/Select patterns[\s\S]*getattr\(control, "invoke", None\)/, 'Edge toolbar actions must prefer semantic UIA Invoke/Select instead of physical input simulation');
assert.match(windowsZoomSource, /zoom-direct-browser-start[\s\S]*find_direct_automation_id_matches\(\s*\(window,\)[\s\S]*zoom-direct-browser-end/, 'The original Edge root may be used for a zero-wait known AutomationId lookup only after popup roots fail');
assert.match(windowsZoomSource, /raw_search_containers = tuple\(owned_windows\)[\s\S]*raw_search_containers \+= \(window,\)[\s\S]*find_bounded_raw_matches\(/, 'The original Edge root must remain bounded for RawView fallback searches');
assert.match(windowsZoomSource, /specification = container\.child_window\(auto_id=automation_id\)[\s\S]*specification\.exists\(timeout=0\)[\s\S]*candidate = specification\.wrapper_object\(\)/, 'Popup AutomationId lookup must probe the WindowSpecification before resolving a wrapper so it cannot inherit pywinauto default waits');
assert.match(windowsZoomSource, /def control_view_descendants\([\s\S]*ControlViewWalker/, 'Zoom lookup must retain a bounded ControlView traversal for semantic popup controls that are absent from RawView');
assert.match(windowsZoomSource, /zoom-control-bounded-start[\s\S]*find_bounded_control_matches\(/, 'Zoom lookup must use the bounded ControlView fallback only after popup AutomationId and RawView lookup fail');
assert.match(windowsZoomSource, /if not zoom_matches and popup_windows:[\s\S]*zoom-popup-descendants-start[\s\S]*find_buttons\([\s\S]*tuple\(popup_windows\)/, 'The final semantic popup fallback may use descendants only on the selected visible popup roots, never on the original Edge window');
