...
assert.match(windowsZoomSource, /all_owned_windows = owned_uia_windows\(desktop, owned_process_id, handle\)[\s\S]*popup_windows = \[[\s\S]*zoom_matches = find_bounded_raw_matches\(/, 'Zoom in lookup must use same-process popup roots and a bounded RawView fallback without probing the full original Edge tree first');
