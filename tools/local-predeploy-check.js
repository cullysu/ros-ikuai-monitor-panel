  ...
  const expression = `(() => {
    ...
    const mobile390ScenarioVisualRecords = mobileOverview390x844
      ? mobileFirstScreenSceneVisualNodes.map((node) => {
        const rect = node.getBoundingClientRect();
        const text = normalize(node.textContent || '');
        const graphicMarks = node.querySelectorAll('svg path, svg polyline, svg line, svg rect, svg circle, [data-overview-chart-point], [data-overview-bar], [data-overview-module-cell], .ro-port-matrix-cell, .ik-mobile-current-rate-snapshot > span > i > u, .ik-mobile-channel-dot, .ik-no-snapshot-chain-node').length;
        const dataToken = [
    ...
    const mobile390AppHomeHeroMetricsOk = !mobileOverview390x844 || Boolean((sectionRoot || mobileFirstScreen)?.querySelector('.ik-mobile-core-facts, .ik-mobile-incident-telemetry, [data-overview-mobile-hero-metrics], [data-overview-mobile-v240-big-numbers], [data-overview-mobile-incident-telemetry]'));
    ...
    const routerMobileChecks = {
      ...
      metrics: routerMobileMetrics.length === 4,
      evidence: routerMobileEvidence.length >= 1 && routerMobileEvidence.length <= 5,
      trafficTruthful: ${JSON.stringify(scaleScenario)} === 'no-snapshot'
        ? routerMobileTraffic?.getAttribute('data-router-mobile-traffic') === 'unavailable' && !/bps|Kbps|Mbps|Gbps/.test(routerMobileText)
      ...
      readonly: routerMobileText.includes('只读监控') && routerMobileText.includes('不会修改路由器配置'),
