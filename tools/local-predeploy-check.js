...
async function inspectSection(cdp, profile, viewport, section, args, scaleScenario) {
  const expression = `(() => {
    ...
    const overviewNoSnapshotDowngradeModule = overviewNoSnapshotBoundaryDegradeModule || overviewNoSnapshotLegacyDowngradeModules[0] || null;
    const overviewNoSnapshotDowngradeText = normalize(overviewNoSnapshotDowngradeModule?.textContent || '');
    const overviewNoSnapshotBoundaryModule = overviewNoSnapshotBoundaryDegradeModule || overviewNoSnapshotLegacyBoundaryModules[0] || null;
    ...
    const overviewNoSnapshotTopbarOk = sectionName !== 'overview' || !isDesktopOverview || !noSnapshotEdge || Boolean(
      ...
      overviewNoSnapshotStatusBarLabels.join('|') === '设备|结论|RouterOS|REST|SSH|最近成功' &&
    ...
    const overviewDesktopScenarioTrendOk = noSnapshotEdge
      ? Boolean(overviewNoSnapshotFlatDetail && overviewNoSnapshotBoundaryDegradeModule)
      : scaleScenario === 'resource-full' || scaleScenario === 'resource-load'
        ? Boolean(
          sectionRoot?.querySelector('[data-overview-density-module="resource-threshold"]') &&
          sectionRoot?.querySelector('[data-overview-density-module="resource-pressure"]')
        )
    ...
    const mobileFlatObjectRow = mobileFlatStatus?.querySelector('[data-overview-mobile-flat-row="object"]') || mobileFlatFactsRow;
    const mobileFlatEvidenceRow = mobileFlatStatus?.querySelector('[data-overview-mobile-flat-row="evidence"]') || mobileFlatStatusRow;
    const mobileFlatEntryRow = mobileFlatStatus?.querySelector('[data-overview-mobile-flat-row="entry"]');
    ...
    const mobileFlatLedgerText = [mobileFlatStatusRowText, mobileFlatDeviceRowText, mobileFlatObjectRowText, mobileFlatEvidenceRowText, mobileFlatEntryRowText].join(' ');
    ...
    const mobileFlatRowCountOk = mobileFlatRows.length >= 5 &&
      ...
      mobileFlatEntryRow &&
      ['status', 'device', 'object', 'evidence', 'entry'].every((key) => mobileFlatRowKeys.includes(key));
    ...
    const overviewFlatMobileContractOk = sectionName === 'overview' && isMobileOverview && Boolean(
      ...
      /结论/.test(mobileFlatStatusText) &&
      /设备/.test(mobileFlatStatusText) &&
      /对象|资源|采集|转发|WAN|接口/.test(mobileFlatStatusText) &&
      /证据/.test(mobileFlatStatusText) &&
      mobileFlatEntryOk &&
      restSshPairPattern.test(mobileFlatStatusText)
    );
    ...
    const overviewNoSnapshotMainVisualOk = sectionName !== 'overview' || !isDesktopOverview || !noSnapshotEdge || Boolean(
      overviewNoSnapshotSummaryVisible &&
      overviewNoSnapshotBoundaryVisible &&
      !overviewNoSnapshotVisibleWanTrend &&
      !/(?:WAN\\s*速率|WAN速率|0\\s*B\\/s)/.test([
        overviewNoSnapshotSummaryModule?.textContent || '',
        overviewDesktopTopText,
        overviewDesktopDetailText,
      ].join(' ')) &&
      /采集链路账本/.test(normalize(overviewNoSnapshotSummaryModule?.textContent || '')) &&
      /默认路由/.test(normalize(overviewNoSnapshotSummaryModule?.textContent || '')) &&
      /失败端点\\s*未记录|失败端点未记录/.test(normalize(overviewNoSnapshotSummaryModule?.textContent || ''))
    );
    ...
    const overviewNoSnapshotLedgerTitlesOk = sectionName !== 'overview' || !isDesktopOverview || !noSnapshotEdge || Boolean(
      ['采集链路账本', '快照账本', '业务展示边界', '最后成功快照摘要', '只读边界 + 降级模块'].every((title) => overviewVisibleDensityModuleText.includes(title))
    );
    ...
    const overviewNoSnapshotRepeatedTokenBudget = isMobileOverview
      ? { noDisplay: 3, notListed: 1, missing: 3, unknown: 6, pending: 5, unavailable: 5, trackedTotal: 10 }
      : { noDisplay: 12, notListed: 1, missing: 4, unknown: 10, pending: 8, unavailable: 8, trackedTotal: 22 };
    ...
    const overviewNoSnapshotDowngradeReasonsOk = sectionName !== 'overview' || !noSnapshotEdge || Boolean(
      isMobileOverview
        ? (
          ...
          /缺少当前路由快照，无法判断默认路由影响|路由快照缺失|默认路由不可判定/.test(overviewNoSnapshotMobileSemanticText)
        )
        : (
          /业务展示边界|展示范围|业务快照/.test(overviewNoSnapshotCurrentText) &&
    ...
    const overviewNoSnapshotDesktopFieldDensityOk = sectionName !== 'overview' || !isDesktopOverview || !noSnapshotEdge || Boolean(
      ...
      overviewNoSnapshotDowngradeModule &&
    ...
    const overviewNoSnapshotFakeDensityTokenCount = countRegex(overviewNoSnapshotSurfaceText, /不展示|不列|缺少|不可判定/g);
    ...
    const overviewNoSnapshotOpsLedgerCopyOk = sectionName !== 'overview' || !isDesktopOverview || !noSnapshotEdge || Boolean(
      ...
      /页面可信等级\\s*(?:采集链路可参考|链路可参考)/.test(overviewNoSnapshotSurfaceText) &&
      /只读边界\\s*不写配置/.test(overviewNoSnapshotSurfaceText) &&
      /业务展示边界|展示范围/.test(overviewNoSnapshotSurfaceText) &&
    ...
    const overviewNoSnapshotNoDuplicateBoundaryOk = sectionName !== 'overview' || !isDesktopOverview || !noSnapshotEdge || Boolean(
      ...
      (overviewNoSnapshotBoundaryDegradeModule ? overviewNoSnapshotBoundaryDegradeModule.querySelectorAll('tbody tr').length >= 12 : false)
    );
    ...
    const overviewNoSnapshotLedgerStructureOk = sectionName !== 'overview' || !isDesktopOverview || !noSnapshotEdge || Boolean(
      ...
      /采集链路账本/.test(overviewNoSnapshotSurfaceText) &&
      /链路层/.test(overviewNoSnapshotSurfaceText) &&
      /最近成功/.test(overviewNoSnapshotSurfaceText) &&
      /失败原因/.test(overviewNoSnapshotSurfaceText) &&
      /下次尝试/.test(overviewNoSnapshotSurfaceText) &&
      /快照账本/.test(overviewNoSnapshotSurfaceText) &&
      /业务展示边界/.test(overviewNoSnapshotSurfaceText) &&
      /展示范围/.test(overviewNoSnapshotSurfaceText) &&
      /WAN/.test(overviewNoSnapshotSurfaceText) &&
      /速率不展示|速率\\s*不展示/.test(overviewNoSnapshotSurfaceText) &&
      /路由\\s*待判定|默认路由\\s*不可判定|默认路由\\s*待判定|默认路由\\s*待判定/.test(overviewNoSnapshotSurfaceText)
    );
    ...
    const overviewNoSnapshotFirstScreenRateForbiddenOk = sectionName !== 'overview' || !noSnapshotEdge || Boolean(
      ...
      !/\\b0\\s*B\\/s\\b/.test(overviewNoSnapshotFirstScreenText)
    );
    ...
    const mobileFirstScreenScenarioCoverageOk = noSnapshotEdge
      ...
      : scaleScenario === 'resource-full'
        ? /资源/.test(firstScreenOverviewText) && /CPU/.test(firstScreenOverviewText) && /MEM|内存/.test(firstScreenOverviewText) && /DISK|磁盘/.test(firstScreenOverviewText) && /采集/.test(firstScreenOverviewText)
        : scaleScenario === 'interfaces-down'
    ...
    const overviewMobileFlatStatusTableOk = sectionName !== 'overview' || !isMobileOverview || Boolean(
      ...
      /结论/.test(mobileFlatStatusText) &&
      /设备/.test(mobileFlatStatusText) &&
      /对象|资源|转发|采集|WAN|接口/.test(mobileFlatStatusText) &&
      /证据/.test(mobileFlatStatusText) &&
      mobileFlatEntryOk &&
    ...
    const overviewStatusBusFixedGrammarOk = sectionName !== 'overview' || !isDesktopOverview || Boolean(
      ...
      (noSnapshotEdge
        ? ['设备', '结论', 'RouterOS', 'REST', 'SSH', '最近成功'].every((label, index) => overviewStatusBarLabels[index] === label)
        : ['设备', '结论', '对象', '影响', '采集', '时间'].every((label, index) => overviewStatusBarLabels[index] === label)) &&
    ...
    const overviewInterfacesForwardingFirstOk = sectionName !== 'overview' || scaleScenario !== 'interfaces-down' || Boolean(
    ...
    const overviewResourceSpecificModuleChecks = {
      resourceThreshold: /resource-threshold|资源阈值|资源均峰/.test(overviewResourceVisibleModuleText),
      connectionPressure: /connection-pressure|resource-pressure|连接压力/.test(overviewResourceVisibleModuleText),
      processServicePressure: /process-service-pressure|process-pressure|service-pressure|进程或服务压力|进程压力|服务压力/.test(overviewResourceVisibleModuleText),
      interfaceThroughputImpact: /interface-throughput-impact|interface-impact|接口吞吐影响|接口吞吐/.test(overviewResourceVisibleModuleText),
    };
    const overviewResourceSpecificModuleCount = overviewVisibleDensityModuleRecords
      .filter((item) => /resource-threshold|resource-pressure|connection-pressure|process-service-pressure|process-pressure|service-pressure|interface-throughput-impact|interface-impact|资源阈值|连接压力|进程或服务压力|进程压力|服务压力|接口吞吐影响/.test([item.module, item.title, item.text].join(' ')))
      .length;
    const overviewResourceSupplementalFactChecks = {
      activeSessions: /active sessions|活动会话/i.test(overviewResourceSpecificText),
      cacheGap: /cache gap|缓存差距|缓存缺口|DNS缓存|DNS 缓存/i.test(overviewResourceSpecificText),
      busiestInterface: /busiest interface|最忙接口|最繁忙接口|接口峰值|接口吞吐影响/i.test(overviewResourceSpecificText),
    };
    const overviewResourceSpecificModulesOk = sectionName !== 'overview' || scaleScenario !== 'resource-full' || !isDesktopOverview || Boolean(
      overviewResourceSpecificModuleCount >= 4 &&
    ...
    const overviewResourceFirstScreenPriorityOk = sectionName !== 'overview' || scaleScenario !== 'resource-full' || Boolean(
      isMobileOverview
        ...
        : (
          ...
          /CPU/.test(overviewResourceDesktopPriorityText) &&
          ...
          /进程或服务压力/.test(overviewResourceDesktopPriorityText) &&
          /接口吞吐影响/.test(overviewResourceDesktopPriorityText) &&
          /DNS缓存/.test(overviewResourceDesktopPriorityText) &&
          /active sessions/.test(overviewResourceDesktopPriorityText) &&
    ...
    const overviewCollectionBusinessTrustCopyOk = sectionName !== 'overview' || scaleScenario !== 'collection-down' || Boolean(
      /业务数字.*上次成功.*可参考.*当前影响未知/.test([
        firstScreenOverviewText,
        mobileTop120Text,
        overviewCombinedCurrentText,
        overviewDesktopDetailText,
    ...
    const overviewStatusBusTripletOk = sectionName !== 'overview' || !isDesktopOverview || Boolean(
      ...
      (
        ...
        (noSnapshotEdge ? /快照缺失|无可用快照|RouterOS/.test(overviewStatusBarText) : /WAN/.test(overviewDesktopTopText)) &&
    ...
    const overviewMobileActionLinksUniqueOk = sectionName !== 'overview' || !isMobileOverview || Boolean(
      ...
      (mobileFlatLinkTexts.length === 0 || firstScreenActionRepeats.actionLabels === 4) &&
      mobileFlatLinkLabelsOk &&
    ...
    const overviewNoSnapshotSemanticOk = sectionName !== 'overview' || !noSnapshotEdge || Boolean(
      ...
      /业务数字不可判定|业务数据不可判定|业务数据不展示|业务数值隐藏|数据可信度\\s*不可判定|当前不可判定|非实时需复核/.test(noSnapshotSemanticText) &&
    ...
    const overviewNoSnapshotSemanticChecks = {
      ...
      hasBusinessUnknown: /业务数字不可判定|业务数据不可判定|业务数据不展示|业务数值隐藏|数据可信度\\s*不可判定|当前不可判定|非实时需复核/.test(noSnapshotSemanticText),
    ...
    const overviewNoSnapshotFiveBlocksOk = sectionName !== 'overview' || !noSnapshotEdge || !isDesktopOverview || Boolean(
      ['采集链路账本', '快照账本', '业务展示边界', '最后成功快照摘要', '只读边界 + 降级模块'].every((label) =>
    ...
    const overviewNoSnapshotGridOk = sectionName !== 'overview' || !noSnapshotEdge || !isDesktopOverview || Boolean(
      ...
      overviewNoSnapshotEvidenceLayoutOk &&
      (overviewNoSnapshotEvidenceTableRows.length >= 4 || overviewNoSnapshotGridItems.length >= 6 || overviewDesktopDetailRows.length >= 6) &&
      /快照账本|快照缺失/.test(noSnapshotStateText) &&
    ...
    if (sectionName === 'overview' && isDesktopOverview && sectionRoot) {
      ...
      overviewDesktopEffectiveHeightOk = effectiveHeight >= effectiveMinHeight && (!noSnapshotEdge || bottomBlank <= 120);
      ...
      overviewNoSnapshotModuleFillOk = sectionName !== 'overview' || !noSnapshotEdge || !isDesktopOverview || Boolean(
        overviewNoSnapshotModuleFillSamples.length >= 5 &&
        overviewNoSnapshotModuleFillRatio >= 0.70
      );
      const overviewResourceFillTargets = scaleScenario === 'resource-full'
        ? overviewDensityModules.filter((node) => {
          ...
          return /resource-threshold|resource-pressure|connection-pressure|process-service-pressure|process-pressure|service-pressure|interface-throughput-impact|interface-impact|资源阈值|连接压力|进程或服务压力|进程压力|服务压力|接口吞吐影响/.test(moduleText) &&
            rect.width > 0 &&
            rect.height > 0 &&
            rect.bottom > 0 &&
            rect.top < window.innerHeight &&
      ...
      overviewResourceModuleFillOk = sectionName !== 'overview' || scaleScenario !== 'resource-full' || !isDesktopOverview || Boolean(
        overviewResourceFillSamples.length >= 4 &&
        overviewResourceModuleFillProbe.minRatio >= 0.76 &&
        overviewResourceSpecificModulesOk
      );
      overviewBlankAreaOk = overviewDesktopRightFillOk && (ratio <= 0.08 || Boolean(
        ...
        overviewNoSnapshotEffectiveVisibleFactCount >= 60 &&
        overviewNoSnapshotModuleFillRatio >= 0.76
      ) || Boolean(
        scaleScenario === 'resource-full' &&
        flatConsoleVisible &&
    ...
    return {
      ...
      overviewNoSnapshotMainVisualProbe: {
        ...
        summaryText: normalize(overviewNoSnapshotSummaryModule?.textContent || '').slice(0, 220),
      },
      overviewNoSnapshotTopbarOk,
