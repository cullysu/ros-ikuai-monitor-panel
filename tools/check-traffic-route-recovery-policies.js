...
const sourcePath = path.join(root, "src", "panel-framework", "sections", "route-recovery", "trafficRecoveryPolicies.ts");
...
const policies = module.exports.trafficRecoveryPolicies;

const expected = ["trafficLoad", "loadAudit", "trafficAudit", "connections"];
...
const requiredEvidence = {
  trafficLoad: {
    partialTerms: ["CPU", "当前样本", "连续性", "不按 0"],
    actionRoutes: ["loadAudit", "readonlyDiagnostics"],
  },
  loadAudit: {
    partialTerms: ["审计序列", "样本", "连续性", "不补为 0"],
    actionRoutes: ["trafficLoad", "readonlyDiagnostics"],
  },
  trafficAudit: {
    partialTerms: ["流量采样", "覆盖", "不按 0", "不代表全量"],
    actionRoutes: ["interfaces", "connections"],
  },
  connections: {
    partialTerms: ["活动连接明细", "完整", "不按 0", "完整连接枚举"],
    actionRoutes: ["terminals", "trafficAudit"],
  },
};
...
if (!policies || typeof policies !== "object" || Array.isArray(policies)) {
  violations.push("trafficRecoveryPolicies must execute to an object");
} else {
...
if (violations.length) {
  console.error("traffic route recovery policy check failed:\n- " + violations.join("\n- "));
  process.exit(1);
}

console.log("traffic route recovery policies passed");
