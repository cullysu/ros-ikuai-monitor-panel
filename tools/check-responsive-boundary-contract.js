 ...
 * Mobile Pulse owns every surface below 1200px; desktop owns 1200px and
...
const contract = "responsive-telemetry-boundary-runtime-v2";
const cells = [
  { id: "phone320", width: 320, height: 568, owner: "mobile-pulse" },
  { id: "phone390", width: 390, height: 844, owner: "mobile-pulse" },
  { id: "phone430", width: 430, height: 932, owner: "mobile-pulse" },
  { id: "landscape667", width: 667, height: 375, owner: "mobile-pulse", landscape: true },
  { id: "landscape844", width: 844, height: 390, owner: "mobile-pulse", landscape: true },
  { id: "tablet768", width: 768, height: 1024, owner: "mobile-pulse", tablet: true },
  { id: "tablet1199", width: 1199, height: 900, owner: "mobile-pulse", tablet: true },
...
function surfaceUrl(baseUrl, owner) {
  const url = new URL(baseUrl);
  url.searchParams.set("surface", owner === "mobile-pulse" ? "mobile" : "desktop");
  return url.toString();
}
...
async function inspectCell(page, cell) {
  ...
  const expected = cell.owner === "mobile-pulse" ? "[data-mobile-pulse-overview]" : "[data-desktop-overview]";
  ...
  const evidence = await page.evaluate(({ expectedOwner, landscape, tablet }) => {
    const originControl = document.querySelector("[data-mobile-pulse-overview]");
    const desktop = document.querySelector("[data-desktop-overview]");
    const nav = document.querySelector("[data-origin-navigation]");
    const active = expectedOwner === "mobile-pulse" ? originControl : desktop;
    ...
    const canvas = originControl;
    const facts = active ? [...active.querySelectorAll(".oc-facts button, .oc-objects li > button")].filter(visible).map(rect) : [];
    ...
    return {
      ...
      originControl: visible(originControl), desktop: visible(desktop), unexpectedMobileMains: originControl ? mobileMains.filter((node) => node !== originControl).length : mobileMains.length, nav: nav ? rect(nav) : null, tabs,
  ...
  if (cell.owner === "mobile-pulse") {
    assert(evidence.originControl && !evidence.desktop, "Mobile Pulse boundary mounted the wrong owner", evidence);
    assert(evidence.canvas && evidence.canvas.width > 0, "Mobile Pulse layout is missing", evidence);
    assert(evidence.tabs.length === 4 && evidence.tabs.every((tab) => tab.label && tab.rect.width >= 44 && tab.rect.height >= 44), "Mobile Pulse navigation must provide four labelled 44px task tabs", evidence);
    ...
    if (cell.id === "phone320") assert(evidence.critical.every((node) => !node.clipped && node.left >= -1 && node.right <= evidence.inner.width + 1), "320px clips or ellipsizes critical Mobile Pulse evidence", evidence);
    if (cell.tablet) assert(evidence.canvas.width >= 240 && evidence.facts.length >= 3 && evidence.facts.every((fact) => fact.width >= 110), "tablet lost Mobile Pulse capability continuity", evidence);
  } else {
    assert(evidence.desktop && !evidence.originControl, "desktop boundary mounted the Mobile Pulse owner", evidence);
  }
  return evidence;
}
...
async function main() {
  ...
  const results = [];
  const surfaceGroups = [
    cells.filter((cell) => cell.owner === "mobile-pulse"),
  ...
  const report = { pass: complete, complete, contract, source: "mobile-telemetry", generatedAt: new Date().toISOString(), commit: identityEnd.commit, artifactKey: identityEnd.artifactKey, worktreeFingerprint: identityEnd.worktreeFingerprint, required: cells.length, completed: results.length, cells: results, freshness: sameIdentity(identityStart, identityEnd), elapsedMs: Date.now() - started };
...
withTimeout("responsive telemetry boundary", () => main(), 180_000).catch((error) => {
  const identity = gitWorktreeIdentity(root);
  const report = { pass: false, complete: false, contract, source: "mobile-telemetry", generatedAt: new Date().toISOString(), commit: identity.commit, artifactKey: identity.artifactKey, worktreeFingerprint: identity.worktreeFingerprint, error: serialise(error) };
  fs.mkdirSync(artifactDir, { recursive: true }); fs.writeFileSync(reportPath, `${JSON.stringify(report, null, 2)}\n`);
