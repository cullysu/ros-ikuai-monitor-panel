...
async function main() {
  ...
  const outJson = path.resolve(arg('--json', 'quick-search.json'));
  const outPng = path.resolve(arg('--png', 'quick-search.png'));
  const width = Number(arg('--width', '1528'));
  const height = Number(arg('--height', '980'));
  const waitMs = Number(arg('--wait', '5000'));
  const port = Number(arg('--port', String(12000 + Math.floor(Math.random() * 900))));
  const userDataDir = path.resolve(arg('--user-data-dir', path.join(process.cwd(), `_edge_search_${port}_${Date.now()}`)));
  ...
  try {
    ...
    const checkExpression = `(() => {
      const button = document.querySelector('[data-quick-search-open]');
      button?.click();
      const input = document.querySelector('.ik-quick-search-input');
      if (!input) return { pass: false, reason: 'input missing' };
      input.value = 'arp';
      input.dispatchEvent(new Event('input', { bubbles: true }));
      const resultText = document.querySelector('.ik-quick-search-results')?.textContent || '';
      input.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true }));
      return {
        pass: Boolean(button && resultText.includes('ARP 监控') && location.hash === '#arp'),
        opened: document.querySelector('.ik-quick-search-overlay')?.classList.contains('is-open') || false,
        resultText,
        hash: location.hash,
        title: document.querySelector('#pageTitle')?.textContent || '',
        quickSearchButtonCount: document.querySelectorAll('[data-quick-search-open]').length
      };
    })()`;
    const result = await send('Runtime.evaluate', { expression: checkExpression, returnByValue: true });
    ...
    if (!report.pass) throw new Error(`Quick search check failed: ${JSON.stringify(report)}`);
