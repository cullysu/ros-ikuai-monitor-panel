...
The only acceptance scope is `public-release`; callers cannot select a route.
...
`tools/check-public-release-readiness.js --release-candidate` accepts only the candidate SHA, external review directory, RouterOS soak report, optional equality assertion for the evidence digest, and soak limits. It deliberately returns `candidateEvidencePass=true`, `publicReleasePass=false`, and `releaseComplete=false`. The repository candidate gate freezes the review bundle and soak once through bounded file handles, validates their shape and exact candidate identity, and recomputes the manifest/evidence digests. Signature trust and promotion authorization are exclusively the external controller's job.
