...
This repository is **not** a release trust authority. In particular, `trusted-reviewers.json` is not read by the release-candidate gate and cannot authorize a release. The commands below are component verifiers: because candidate code can be changed, their success can establish only `candidateEvidencePass`, never promotion authorization or release completion.
...
The Accessibility/Interaction record must cover every operational route through real assistive-technology testing. It must record, at minimum, the assistive technology and version, operating system, browser or host, device context, interaction modes exercised, exact routes covered, per-route outcomes, exact candidate identity, and paths to the supporting evidence. Automated checks, source inspection, screenshots, or agent review alone are not real AT acceptance. Missing metadata, incomplete route coverage, candidate mismatch, or any non-pass outcome fails closed.
...
`tools/check-public-release-readiness.js --release-candidate` accepts only the candidate SHA, external review directory, RouterOS soak report, optional equality assertion for the evidence digest, and soak limits. It must return `publicReleasePass=false` and `releaseComplete=false`; `candidateEvidencePass=true` is allowed only when the full candidate-evidence contract is actually satisfied. The repository candidate gate freezes the review bundle and soak once through bounded file handles, validates their shape and exact candidate identity, and recomputes the manifest/evidence digests. Signature trust and promotion authorization are exclusively the external controller's job.
