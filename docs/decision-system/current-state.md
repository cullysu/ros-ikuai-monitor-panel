...
- supersededBy: `null`
- updatedAt: 2026-08-05T12:30:00+08:00
...
## Current decision record: Step 868

- observed: after Step867, exact-SHA Overview rerun found 14 failures, all `patrolActions=false` in mobile incident scenarios. The UI had three actions split between the primary `.mp-actions` section and the explicitly marked secondary context list, while the acceptance probe counted only the first section.
- decision: aggregate `.mp-actions` and `[data-mobile-incident-follow-up-context]` buttons in the Overview probe, preserving the original three-action and route-completeness requirements. Do not weaken the gate or alter product visibility.
- visual disposition: the product/runtime split remains valid; the failed matrix is a test-semantic mismatch, not a visual pass. The probe fix must be committed and all exact-SHA artifacts regenerated before current-SHA scoped Visual/Product/Interaction review.
...
- report truth: current matrix families must not share a root aggregate path. The Step868 tracked probe/governance change invalidates the Step867 exact-SHA reports; report quarantine, truth and readiness must be rerun after commit; historical and failed reports are inventory evidence only and cannot become current release input.
...
- R07 mobile visual maturity, R09 tablet task efficiency, R10 desktop density and R14 cross-surface grammar have independent scoped evidence; the local 375px visual P1 is fixed, but Visual must re-sign the new exact-SHA artifact family after commit and Product remains conditional. The Step868 matrix failure is kept visible until the rebind passes.
