...
- validForCommit: Step964 mobile 12px text-floor repair passes focused runtime; exact clean replay remains pending
- currentHandoffForStep: `964`
...
- Preserve the active Step964 style owners and 12px mobile small-text floor: `patrol-next.css`, `incidents-next.css`, `shell-next.css`, `motion.css` and `desktop-next.css`; do not restore retired `layout.css` or `desktop-overview-recovered.css`.
...
| State matrix | `pending replay` | The first 1206979 matrix is red historical evidence; all cells must replay after Step964. |
...
Commit Step964, then bind every complete local gate, matrix and independent review to the resulting clean exact SHA. Continue all local work; do not mark the task blocked merely because real external attestations or publication evidence are still pending.
